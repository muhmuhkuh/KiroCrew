 ...
 * clipboard) plus a confirmation that has to go away again, and the confirmation
 * is gated on the shared `copyToClipboard` helper's boolean result — a `false`
 * (clipboard blocked, permission denied, insecure context) must render NO
 * confirmation, because a tick over an unchanged clipboard is worse than no
 * affordance at all. Each of those behaviours is easy to break silently, so
 * each is pinned here.
 */
...
describe('CopyBtn', () => {
  ...
  it('writes the exact text it was given and confirms, then reverts', async () => {
    ...
    await waitFor(() => expect(copyToClipboard).toHaveBeenCalledWith('217681647555'))
  ...
  it('renders no confirmation when the copy fails', async () => {
    // The shared helper never rejects — it resolves `false` on failure (denied
    // permission, insecure context, no execCommand fallback either). The button
    // must gate its confirmation on that boolean rather than assuming success.
