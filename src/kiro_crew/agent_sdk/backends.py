...
BASELINE_SELECTABLE_BACKENDS: frozenset[str] = frozenset(
    {ACP_BACKEND_KIRO, ACP_BACKEND_CLAUDE, ACP_BACKEND_KAS, ACP_BACKEND_CODEX, ACP_BACKEND_PI}
)
...
ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION = frozenset({ACP_BACKEND_CLAUDE, ACP_BACKEND_CODEX, ACP_BACKEND_PI})
...
# never advertised.
ACP_BACKENDS_EFFORT_VIA_CONFIG_OPTION = frozenset({ACP_BACKEND_CLAUDE, ACP_BACKEND_CODEX, ACP_BACKEND_PI})
...
_MODEL_REGISTRY_NAMESPACE_BY_BACKEND: dict = {
    ...
    # Pi model ids are provider-qualified (for example, ``anthropic/model``).
    # They must reach pi-acp verbatim rather than being normalized to kiro-cli's
    # ACP spelling.
    ACP_BACKEND_PI: "pi",
}
...
ACP_BACKEND_ROUTING: dict = {
    ...
    ACP_BACKEND_PI: Routing.UNVERIFIED,
}
