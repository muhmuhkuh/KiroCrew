...
const MODEL_TOKENS: Record<string, number> = Object.fromEntries(
  Object.entries(modelTokensRaw).filter(([k]) => !k.startsWith("_")) as [
    string,
    number,
  ][],
);
...
for (const m of readCachedModels() ?? [])
  learnWindow(m.name, m.contextWindow ?? 0);
...
export class AcpAdapter implements ProviderAdapter {
  ...
  readonly labels: ProviderLabels = {
    ...
    warmPoolDescription:
      "Pre-spawn ACP CLI processes for instant session start.",
    ...
    return {
      sessions: {
        ...
        today: {
          ...
          toolCalls: s.today.tool_calls,
        },
        thisWeek: {
          sessions: s.this_week.sessions,
          messages: s.this_week.messages,
          toolCalls: s.this_week.tool_calls,
        },
        thisMonth: {
          sessions: s.this_month.sessions,
          messages: s.this_month.messages,
          toolCalls: s.this_month.tool_calls,
        },
      ...
      billing: b.plan
        ? {
            ...
            unit: "credits",
            resets: b.resets,
            percentUsed: b.credits_plan
              ? Math.round(((b.credits_used ?? 0) / b.credits_plan) * 100)
              : undefined,
          }
        : null,
    };
  ...
  async fetchProviderHooks(): Promise<
    Record<string, NormalizedProviderHook[]>
  > {
    ...
    try {
      ...
      const result = models.map((m: RawModel) => {
        ...
        return {
          ...
          contextWindow:
            reported || MODEL_TOKENS[m.model_name] || DEFAULT_CONTEXT,
    ...
    return [
      {
        id: "normal",
        label: "Normal",
        description: "Ask before each tool execution",
      },
      {
        id: "trust_reads",
        label: "Trust Reads",
        description: "Auto-approve read operations, ask for writes",
      },
      {
        id: "trust",
        label: "Trust All",
