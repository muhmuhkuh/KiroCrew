    ...
    <>
      <div className="px-3 pt-2">
        <div className="flex items-center gap-1.5 mb-1.5 text-[12px] font-semibold text-muted uppercase tracking-[.05em]">
          <ArrowUpDown size={12} />{" "}
          {i18nT("apps.issueRadar.components.filtersSection.sort")}
        </div>
        <div className="flex flex-col gap-0.5">
          {SORT_FIELDS.filter(
            (f) => active.provider === "jira" || f.key !== "priority",
          ).map((f) => {
            const isActive =
              f.key ===
      ...
      <div className="px-3 pt-5">
        <div className="flex items-center mb-1.5">
          ...
          {anyFilterActive && (
            ...
            >
              <X size={11} />{" "}
              {i18nT("apps.issueRadar.components.filtersSection.clear")}
            </button>
          )}
        </div>
        <div className="flex flex-col gap-0.5">
          ...
          <FilterRow
            label={i18nT("apps.issueRadar.components.filtersSection.open")}
          ...
          />
          <FilterRow
            label={i18nT("apps.issueRadar.components.filtersSection.closed")}
          ...
          <FilterRow
            label={i18nT(
              "apps.issueRadar.components.filtersSection.requested_by_me",
            )}
            active={requestedByMe}
            disabled={!me}
            onToggle={toggleRequestedByMe}
          />
          <FilterRow
            label={i18nT(
              "apps.issueRadar.components.filtersSection.assigned_to_me",
            )}
            active={assignedToMe}
            disabled={!me}
          ...
          <FilterRow
            label={i18nT(
              "apps.issueRadar.components.filtersSection.created_by_member",
            )}
            active={createdByMember}
            disabled={!hasMemberIssues}
            disabledHint={i18nT(
              "apps.issueRadar.components.filtersSection.no_repo_members_found_among_these_issues",
            )}
            onToggle={toggleCreatedByMember}
          />
        </div>
        {active.provider === "jira" && sortedIssueStatuses.length > 0 && (
          <div className="mt-4">
            <div className="flex items-center gap-1.5 mb-1.5 text-[12px] font-semibold text-muted uppercase tracking-[.05em]">
              <CircleDot size={12} />{" "}
              {i18nT("apps.issueRadar.components.filtersSection.status")}
            </div>
            <div className="flex flex-col gap-0.5">
              {sortedIssueStatuses.map(({ name, count }) => {
                const selected = selectedStatuses.has(name);
                return (
                  <button
                    key={name}
                    type="button"
