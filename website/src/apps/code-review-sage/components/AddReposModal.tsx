    ...
    <li>
      <button
        ...
        aria-label={
          pinned
            ? i18nT(
                "apps.codeReviewSage.components.addReposModal.already_added",
                { repo: fullName },
              )
            : i18nT("apps.codeReviewSage.components.addReposModal.add_repo", {
                repo: fullName,
              })
        }
      ...
      >
        ...
        {pinned ? (
          <span className="flex-shrink-0 inline-flex items-center gap-1 text-[11px] text-ok">
            ...
            {i18nT("apps.codeReviewSage.components.addReposModal.added")}
          </span>
        ) : (
          <Plus
            size={13}
...
function SectionHead({ title, hint }: { title: string; hint?: string }) {
  return (
    <div className="flex items-baseline gap-2 mt-1">
      ...
      {hint && (
        <span className="text-[11.5px] text-muted opacity-70">{hint}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-border bg-bg-elevated px-3 py-2.5 text-[12.5px] leading-[1.6]">
      <span className="inline-flex items-center gap-1.5 text-warn font-medium">
        ...
        {i18nT(
          "apps.codeReviewSage.components.addReposModal.github_cli_not_ready",
        )}
      </span>
      <div className="text-muted mt-1">
        ...
        <div className="mt-1">
          <code className="font-mono">
            {i18nT(
              "apps.codeReviewSage.components.addReposModal.gh_auth_login",
            )}
          </code>
        </div>
      </div>
      {message && (
        <div className="text-muted opacity-70 mt-1 break-words">{message}</div>
      )}
    </div>
  );
}
...
export default function AddReposModal({ onClose }: { onClose: () => void }) {
  ...
  const recentKeys = new Set(
    (recent?.repos ?? []).map((r) => r.full_name.toLowerCase()),
  );
  const mineRows = (mine?.repos ?? []).filter(
    (r) => match(r.full_name) && !recentKeys.has(r.full_name.toLowerCase()),
  );
    ...
    <AnimatePresence>
      ...
      <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
        <Clickable
          ...
          aria-label={i18nT(
            "apps.codeReviewSage.components.addReposModal.close_add_repos_dialog",
          )}
        />
        <motion.div
          ref={dialogRef}
          ...
          aria-modal="true"
          aria-label={i18nT(
            "apps.codeReviewSage.components.addReposModal.add_repos",
          )}
          tabIndex={-1}
        ...
        >
          <header className="px-4 md:px-6 pt-5 pb-4 border-b border-border flex-shrink-0">
            ...
            <p className="text-[12.5px] text-muted mt-1.5 leading-[1.5]">
              {i18nT(
                "apps.codeReviewSage.components.addReposModal.pick_from_the_repos_you_can_reach_or_type_one_in",
              )}
            </p>
            <button
              onClick={requestClose}
              aria-label={i18nT(
                "apps.codeReviewSage.components.addReposModal.close",
              )}
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 rounded-lg border border-border bg-bg-elevated px-2.5 flex-1 min-w-0 transition-colors focus-within:border-accent">
                ...
                <input
                  ...
                  aria-label={i18nT(
                    "apps.codeReviewSage.components.addReposModal.repository_or_pull_request_url_or_owner_repo",
                  )}
                  placeholder={i18nT(
                    "apps.codeReviewSage.components.addReposModal.owner_repo_a_repo_url_or_paste_a_pull_request_li",
                  )}
              ...
              >
                ...
                {i18nT("apps.codeReviewSage.components.addReposModal.add")}
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 rounded-lg border border-border bg-bg-elevated px-2.5 flex-1 min-w-0 transition-colors focus-within:border-accent">
                ...
                <input
                  ...
                  aria-label={i18nT(
                    "apps.codeReviewSage.components.addReposModal.filter_your_repos",
                  )}
                  placeholder={i18nT(
                    "apps.codeReviewSage.components.addReposModal.filter_your_repos",
                  )}
              ...
              <button
                ...
                disabled={loading}
                title={i18nT(
                  "apps.codeReviewSage.components.addReposModal.re_read_your_repos_from_github",
                )}
                aria-label={i18nT(
                  "apps.codeReviewSage.components.addReposModal.refresh_your_repos",
                )}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto scrollbar-none px-4 md:px-6 pb-5 flex flex-col gap-2.5">
            {setupRequired && (
              <GhNotice message={recent?.error ?? mine?.error} />
            )}
            ...
            {loading && !setupRequired && (
              <>
                <div className="inline-flex items-center gap-1.5 text-[12.5px] text-muted">
                  ...
                  />
                  {i18nT(
                    "apps.codeReviewSage.components.addReposModal.reading_your_repos_from_github",
                  )}
                </div>
            ...
            {pinnedRepos.length > 0 && (
              <>
                <SectionHead
                  title={i18nT(
                    "apps.codeReviewSage.components.addReposModal.your_repos",
                  )}
                  hint={i18nT(
                    "apps.codeReviewSage.components.addReposModal.pinned_to_the_sidebar",
                  )}
                />
                <ul className="flex flex-col gap-1.5 list-none p-0 m-0">
                  {pinnedRepos.map((r) => (
                    <li
                      key={`${r.host ?? "github.com"}/${r.provider ?? "github"}/${r.owner}/${r.repo}`}
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          unpinRepo(r.owner, r.repo, r.provider, r.host)
                        }
                        aria-label={i18nT(
                          "apps.codeReviewSage.components.addReposModal.remove_repo",
                          { repo: r.owner ? `${r.owner}/${r.repo}` : r.repo },
                        )}
                      ...
                      >
                        ...
                        {i18nT(
                          "apps.codeReviewSage.components.addReposModal.remove",
                        )}
                      </button>
                    </li>
                  ))}
              ...
              <>
                {recentRows.length > 0 && (
                  <>
                    <SectionHead
                      title={i18nT(
                        "apps.codeReviewSage.components.addReposModal.recently_worked_on",
                      )}
                      hint={i18nT(
                        "apps.codeReviewSage.components.addReposModal.based_on_recent_github_activity",
                      )}
                    />
                    <ul className="flex flex-col gap-1.5 list-none p-0 m-0">
                      {recentRows.map((r) => (
                        <RepoRow
                          ...
                          meta={i18nT(
                            "apps.codeReviewSage.components.addReposModal.contributed_events",
                            {
                ...
                {mineRows.length > 0 && (
                  <>
                    <SectionHead
                      title={i18nT(
                        "apps.codeReviewSage.components.addReposModal.all_your_repos",
                      )}
                      hint={
                        mine?.truncated
                          ? i18nT(
                              "apps.codeReviewSage.components.addReposModal.newest_100_by_push_date",
                            )
                          : undefined
                      }
                    />
                      ...
                      {mineRows.map((r) => (
                        <RepoRow
                          ...
                          meta={
                            r.pushed_at
                              ? i18nT(
                                  "apps.codeReviewSage.components.addReposModal.pushed_age",
                          ...
                          badges={
                            <span className="flex-shrink-0 inline-flex items-center gap-1.5">
                              {r.private && (
                                <span
                                  title={i18nT(
                                    "apps.codeReviewSage.components.addReposModal.private",
                                  )}
                              ...
                              {r.archived && (
                                <span className="text-[10.5px] px-1.5 py-0.5 rounded-full bg-bg-elevated border border-border text-muted">
                                  {i18nT(
                                    "apps.codeReviewSage.components.addReposModal.archived",
                                  )}
                                </span>
                              )}
                              {!r.can_push && (
                                <span
                                  title={i18nT(
                                    "apps.codeReviewSage.components.addReposModal.read_only_access",
                                  )}
                                  className="text-[10.5px] px-1.5 py-0.5 rounded-full bg-bg-elevated border border-border text-muted"
                                >
                                  {i18nT(
                                    "apps.codeReviewSage.components.addReposModal.read_only",
                                  )}
                                </span>
                              )}
                            </span>
                ...
                {recentRows.length === 0 && mineRows.length === 0 && (
                  <EmptyState
                    ...
                    title={
                      q
                        ? i18nT(
                            "apps.codeReviewSage.components.addReposModal.no_repos_match_filter",
                          )
                        : i18nT(
                            "apps.codeReviewSage.components.addReposModal.no_repos_for_account",
                          )
                    }
                    hint={i18nT(
                      "apps.codeReviewSage.components.addReposModal.add_one_by_url_above",
                    )}
                  />
                )}
                ...
                {(mine?.truncated || recent?.truncated) && (
                  <div className="text-[11.5px] text-muted opacity-80 leading-[1.5] mt-1">
                    {i18nT(
                      "apps.codeReviewSage.components.addReposModal.these_lists_are_capped_so_a_repo_you_have_not_to",
                    )}
                  </div>
                )}
              </>
