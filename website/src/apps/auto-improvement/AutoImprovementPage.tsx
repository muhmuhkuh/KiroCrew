...
function PrRow({ finding, repo }: { finding: Finding; repo: string }) {
    ...
    const { data, refetch, isFetching } = useQuery({
        ...
        queryFn: () =>
            getJson<PrStatus>(`${API}${prStatusQuery(url as string)}`),
        enabled: Boolean(url),
        ...
        <tr className="border-t border-border">
            <td className="py-2 pr-3 align-top">
                ...
                >
                    ...
                    {data?.number
                        ? `#${data.number}`
                        : i18nT("autoImprovement.draftPr")}
                    <ExternalLink className="lucide-inline" size={12} />
                </a>
            </td>
            <td className="py-2 pr-3 align-top text-[13px]">
                {data?.title || finding.target}
            </td>
            <td className="py-2 pr-3 align-top">
                {data?.ok === false ? (
                    <Badge variant="muted">
                        {i18nT("autoImprovement.statusUnavailable")}
                    </Badge>
                ) : (
            ...
            <td className="py-2 align-top">
                <div className="flex items-center gap-1">
                    ...
                    >
                        <RefreshCw
                            className={`lucide-inline ${isFetching ? "animate-spin" : ""}`}
                            size={14}
                        />
                    </Clickable>
                    <Clickable
                    ...
                    >
                        <MessageSquare
                            className={`lucide-inline ${busy ? "opacity-50" : ""}`}
                            size={14}
                        />
                    </Clickable>
                </div>
...
export default function AutoImprovementPage() {
    ...
    const repo = String(
        config?.target_display || config?.target_url || "repository",
    );
    ...
    // either does.
    const branch =
        String(config?.branch || "").replace(/^origin\//, "") || "default";
    ...
    const commitFinding = useMutation({
        mutationFn: async (fp: string) => {
            const res = await fetch(
                `${API}/findings/${encodeURIComponent(fp)}/commit`,
                {
                    method: "POST",
                },
            );
        ...
        <div className="mx-auto flex w-full max-w-[1100px] flex-col gap-4 p-4">
            <header className="flex items-center gap-2">
                ...
                <h1 className="text-[17px] font-semibold">
                    {i18nT("autoImprovement.title")}
                </h1>
            </header>
            <p className="text-[13px] text-muted">
                {i18nT("autoImprovement.intro")}
            </p>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <StatCard
                    ...
                    value={
                        calibrated
                            ? i18nT("autoImprovement.calibrated")
                            : i18nT("autoImprovement.uncalibrated")
                    }
                    colorClass={calibrated ? undefined : "text-warn"}
                />
                <StatCard
                    label={i18nT("autoImprovement.findingsStat")}
                    value={findings.length}
                />
                <StatCard
                    label={i18nT("autoImprovement.keptStat")}
                    value={drafted.length}
                />
                <StatCard
                    label={i18nT("autoImprovement.prStat")}
                    value={withPr.length}
                />
            </div>
            ...
            <Card>
                ...
                {calibrated ? (
                    <p className="text-[13px] text-muted">
                        {ruler?.primary?.label ||
                            ruler?.primary?.name ||
                            i18nT("autoImprovement.rulerReady")}
                        {ruler?.noiseBand?.value !== undefined
                    ...
                    <p className="text-[13px] text-muted">
                        {i18nT("autoImprovement.rulerNotReady")}
                    </p>
                )}
            </Card>
                        ...
                        <thead>
                            <tr className="text-[11px] uppercase tracking-wide text-muted">
                                <th className="pb-2 pr-3 font-medium">
                                    {i18nT("autoImprovement.colPr")}
                                </th>
                                <th className="pb-2 pr-3 font-medium">
                                    {i18nT("autoImprovement.colChange")}
                                </th>
                                <th className="pb-2 pr-3 font-medium">
                                    {i18nT("autoImprovement.colVerdict")}
                                </th>
                                <th className="pb-2 pr-3 font-medium">
                                    {i18nT("autoImprovement.colChecks")}
                                </th>
                                <th className="pb-2 font-medium">
                                    {i18nT("autoImprovement.colActions")}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            <Card>
                <div className="flex items-center justify-between gap-2">
                    <CardTitle>
                        {i18nT("autoImprovement.findingsHeading")}
                    </CardTitle>
              ...
              when either does, so the scope is always visible. */}
                ...
                {findings.length === 0 ? (
                    <p className="text-[13px] text-muted">
                        {i18nT("autoImprovement.noFindings")}
                    </p>
                ) : (
                    <ul className="flex flex-col gap-1">
                        {findings.slice(0, FINDINGS_SHOWN).map((f) => (
                            <li
                                key={f.fp}
                                className="flex flex-col gap-1 py-1 text-[13px]"
                            >
                                <div className="flex items-center justify-between gap-2">
                      ...
                      action, so it should not require hunting for a control. */}
                                    <Clickable
                                        onClick={() =>
                                            setExpanded(
                                                expanded === f.fp ? null : f.fp,
                                            )
                                        }
                                        aria-label={i18nT(
                                            "autoImprovement.toggleDetail",
                                        )}
                                    ...
                                    >
                                        {expanded === f.fp ? (
                                            <ChevronDown
                                                className="lucide-inline shrink-0"
                                                size={13}
                                            />
                                        ) : (
                                            <ChevronRight
                                    ...
                                    <span className="flex shrink-0 items-center gap-2">
                                        {prUrlOf(f) ? (
                                            ...
                                            >
                                                ...
                                                />
                                                {i18nT(
                                                    "autoImprovement.viewPr",
                                                )}
                                            </a>
                                        ) : null}
                                        <span className="text-muted">
                                            {STATUS_LABEL_KEY[
                                                f.status as keyof typeof STATUS_LABEL_KEY
                                            ]
                                                ? i18nT(
                                                      STATUS_LABEL_KEY[
                                                          f.status as keyof typeof STATUS_LABEL_KEY
                        ...
                        ledger stores a bare sha here, so this is a link out to the forge. */}
                                        {f.status === COMMITTED_STATUS &&
                                        commitUrlOf(f, repo, provider, host) ? (
                                            <a
                                                href={
                                                    commitUrlOf(
                                                        f,
                                        ...
                                        {DRAFTED_STATUSES.has(f.status) ? (
                                            <Clickable
                                                ...
                                                disabled={
                                                    commitFinding.isPending
                                                }
                                                aria-label={i18nT(
                                                    "autoImprovement.commitFinding",
                                                )}
                                                title={i18nT(
                                                    "autoImprovement.commitFinding",
                                                )}
                            ...
                            harmless Discuss icon next to it. */}
                                        ...
                                        <Clickable
                                            ...
                                            aria-label={i18nT(
                                                "autoImprovement.discussFinding",
                                            )}
                                            title={i18nT(
                                                "autoImprovement.discussFinding",
                                            )}
                    ...
                    unrelated finding. Mirrors what `SetupPanel` already does for clone. */}
                                {commitFinding.isError &&
                                commitFinding.variables === f.fp ? (
                                    <p className="text-[12px] text-warn">
                                        {(commitFinding.error as Error)
                                            ?.message ||
                                            i18nT(
                                                "autoImprovement.commitFailed",
                                            )}
                                    </p>
                                ) : null}
                                {expanded === f.fp ? (
                                    <FindingDetail fp={f.fp} />
                                ) : null}
                            </li>
                        ))}
                    </ul>
