...
export function relativeAge(iso?: string): string {
 ...
 if (mins < 60)
  return i18nT("apps.codeReviewSage.lib.format.minutes_ago", { count: mins });
 const hrs = Math.floor(mins / 60);
 if (hrs < 24)
  return i18nT("apps.codeReviewSage.lib.format.hours_ago", { count: hrs });
...
export const TERMINAL_PHASES: ReadonlySet<string> = new Set<ChangePhase>([
 "done",
 "failed",
 "cancelled",
]);
...
export function phaseLabel(phase: string): string {
 switch (phase) {
  case "queued":
   return i18nT("apps.codeReviewSage.lib.format.phase_queued");
  case "reviewing":
   return i18nT("apps.codeReviewSage.lib.format.phase_reviewing");
  case "done":
   return i18nT("apps.codeReviewSage.lib.format.phase_done");
  case "failed":
   return i18nT("apps.codeReviewSage.lib.format.phase_failed");
  case "cancelled":
   return i18nT("apps.codeReviewSage.lib.format.phase_cancelled");
  default:
   return phase
...
export function repoOfRun(run: {
 ...
 try {
  ...
  const end = marker >= 0 ? marker : parts.indexOf("pull");
  const repoParts = end >= 0 ? parts.slice(0, end) : [];
  if (repoParts.length >= 1) {
   const slug = repoParts.join("/");
   return url.hostname === "github.com" || url.hostname === "gitlab.com"
    ? slug.toLowerCase()
    : `${url.hostname}/${slug}`.toLowerCase();
  }
 } catch {
  /* not a URL */
 }
...
export function failureReason(
 run: {
  ...
  progress?: Record<
   string,
   { phase?: string; error?: string; reason?: string }
  >;
 },
 changeId?: string,
): { text: string; raw: string } | null {
 if (effectiveRunStatus(run) !== "error" && run.status !== "interrupted")
  return null;
 ...
 if (!raw)
  return {
   text: i18nT("apps.codeReviewSage.lib.format.cause_unknown"),
   raw: "",
  };
 ...
 if (/no kiro-cli executable/.test(lower)) {
  return {
   text: i18nT("apps.codeReviewSage.lib.format.cause_no_agent_cli"),
   raw,
  };
 }
 if (/is not importable/.test(lower)) {
  return {
   text: i18nT("apps.codeReviewSage.lib.format.cause_runtime_not_importable"),
   raw,
  };
 }
 if (/runtime_unavailable|agent runtime is unavailable/.test(lower)) {
  return {
   text: i18nT("apps.codeReviewSage.lib.format.cause_runtime_unavailable"),
   raw,
  };
 }
 ...
 if (/never completed the review|review_record_incomplete/.test(lower)) {
  return {
   text: i18nT("apps.codeReviewSage.lib.format.cause_record_incomplete"),
   raw,
  };
 }
 ...
 if (/review_failed/.test(lower)) {
  return {
   text: i18nT("apps.codeReviewSage.lib.format.cause_turn_failed"),
   raw,
  };
 }
 ...
 const byToken: Record<string, string> = {
  runtime_unavailable:
   "apps.codeReviewSage.lib.format.cause_runtime_unavailable",
  no_review_recorded: "apps.codeReviewSage.lib.format.cause_no_record",
  review_record_incomplete:
   "apps.codeReviewSage.lib.format.cause_record_incomplete",
 };
 ...
 *  change URL. The backend already validates self-hosted GitLab URLs; pass that
 *  exact host to the shared parser so this does not maintain a second grammar.
 */
export function sageSourceLink(url: string): PullRequestLink | null {
 try {
  ...
  const gitlabHosts =
   host === "github.com" || host === "gitlab.com" ? [] : [parsed.host];
  const link = parseSourceLinkUrl(url, gitlabHosts);
  return link?.kind === "change" ? link : null;
 } catch {
  return null;
 }
}
