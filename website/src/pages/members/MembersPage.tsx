...
const SOURCE_CHIPS: readonly Exclude<MemberSourceFilter, "all">[] = [
  "mine",
  "builtin",
  "package",
];
...
const SOURCE_CHIP_LABEL_KEY: Record<
  Exclude<MemberSourceFilter, "all">,
  string
> = {
...
const SOURCE_CHIP_TITLE_KEY: Record<
  Exclude<MemberSourceFilter, "all">,
  string
> = {
...
const DRIVING_STATUS: Record<
  TabStatus,
  { cls: string; text: string; label: string; spoken: boolean }
> = {
  permission: {
    cls: "fill-warn text-warn",
    text: "text-warn",
    label: "pages.chatSidebar.needs_approval",
    spoken: true,
  },
  question: {
    cls: "fill-info text-info",
    text: "text-info",
    label: "pages.chatSidebar.needs_your_answer",
    spoken: true,
  },
  running: {
    cls: "fill-ok text-ok",
    text: "text-ok",
    label: "pages.membersPage.drawer_working",
    spoken: false,
  },
  unread: {
    cls: "fill-muted text-muted",
    text: "text-muted",
    label: "pages.membersPage.driving_idle",
    spoken: false,
  },
  idle: {
    cls: "fill-muted text-muted",
...
const loadRosterWidth = () =>
  loadColumnWidth(ROSTER_WIDTH_KEY, ROSTER_MIN, ROSTER_MAX, ROSTER_DEFAULT);
...
export default function MembersPage() {
  ...
  const [gone, setGone] = useState<{ name: string; shown: string } | null>(
    null,
  );
  ...
  const liveRunning = useMemo(() => {
    ...
    for (const s of liveSlots)
      if (s.mode === "member") byKey[s.key] = !!s.running;
    return byKey;
  }, [liveSlots]);
  ...
  const [starredOnly, setStarredOnly] = usePersistedBool(
    STARRED_ONLY_KEY,
    false,
  );
  const [rawSourceFilter, setRawSourceFilter] = usePersistedString(
    SOURCE_FILTER_KEY,
    "all",
  );
  ...
  const pickSource = useCallback(
    (next: MemberSourceFilter) => {
      ...
      setRawSourceFilter((prev) =>
        parseSourceFilter(prev) === next ? "all" : next,
      );
    },
    [setRawSourceFilter],
  );
  ...
  const patchStar = useCallback(
    (name: string, starred: boolean) =>
      queryClient.setQueryData<MemberRosterRow[]>(
        MEMBERS_ROSTER_QUERY_KEY,
        (rows) => rows?.map((r) => (r.name === name ? { ...r, starred } : r)),
      ),
    [queryClient],
  );
  ...
  const starredCount = useMemo(
    () => members.filter((m) => !!m.starred).length,
    [members],
  );
  ...
  const sourceCounts = useMemo(() => {
    const out: Record<Exclude<MemberSourceFilter, "all">, number> = {
      mine: 0,
      builtin: 0,
      package: 0,
    };
    for (const m of members) {
      for (const chip of SOURCE_CHIPS)
        if (matchesSource(m, chip)) out[chip] += 1;
    }
    return out;
  }, [members]);
  ...
  const orderedMembers = useMemo(() => {
    ...
    const sameMembership =
      prev.length === byName.size && prev.every((n) => byName.has(n));
    const names = sameMembership
      ? prev
      : [...members]
          .sort(
            (a, b) =>
              (b.last_active_ts ?? 0) - (a.last_active_ts ?? 0) ||
    ...
    return names
      .map((n) => byName.get(n))
      .filter((m): m is MemberRosterRow => !!m);
  }, [members]);
  ...
  const committedDrivingRef = useRef<{ member: string; keys: string[] }>({
    member: "",
    keys: [],
  });
  const drivingSessions = useMemo(() => {
    if (!activeMemberKey) return [];
    const mine = liveSlots.filter(
      (s) => !!s.created_by && s.created_by === activeMemberKey,
    );
    ...
    return keys
      .map((k) => byKey.get(k))
      .filter((s): s is (typeof mine)[number] => !!s);
  ...
  const visibleDriving = drivingExpanded
    ? drivingSessions
    : drivingSessions.slice(0, DRIVING_VISIBLE);
  ...
  const activityLoading =
    activityQuery.data === undefined && !activityQuery.isError;
  const activityError =
    activityQuery.data === undefined && activityQuery.isError;
  ...
  const wakeLoaded =
    wakeFailed || wakeSources.every((q) => q.data !== undefined);
  ...
  const wakeJobs = useMemo(
    () =>
      active && wakeJobsAll
        ? wakeJobsAll.filter((j) =>
            wakesCrew(j, active.name, active.name === wakeDefaultAgent),
          )
        : [],
  ...
  const { todayCount, weekCount, todayFloorTs, weekFloorTs } = useMemo(() => {
    ...
    return {
      todayCount: today,
      weekCount: week,
      todayFloorTs: todayFloor,
      weekFloorTs: weekFloor,
    };
  }, [activeEntries]);
  ...
  const oldestTs = activeEntries.length
    ? activeEntries[activeEntries.length - 1].ts
    : 0;
  ...
  const patrol = useMemo(() => {
    ...
    for (const lp of data?.loops || [])
      if (lp?.slot_key) loops[lp.slot_key] = lp;
    return {
  ...
  const activePatrol = activeMemberKey
    ? patrol.loops[activeMemberKey]
    : undefined;
  ...
  const setThreadOutcome = useCallback(
    (
      name: string,
      update: (prev: MemberThreadOutcome | undefined) => MemberThreadOutcome,
    ) =>
      queryClient.setQueryData<MemberThreadOutcome>(
        memberThreadQueryKey(name),
        update,
      ),
    [queryClient],
  );
      ...
      if (m.slot_key !== r.slot_key) {
        queryClient.setQueryData<MemberRosterRow[]>(
          ...
          (rows) =>
            rows?.map((row) =>
              row.name === m.name
                ? { ...row, slot_key: r.slot_key, bound: true }
                : row,
            ),
        );
      }
  ...
  useEffect(() => {
    if (!connected) return;
    if (hadConnectionRef.current && activeRowRef.current)
      postThread(activeRowRef.current);
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      if (urlMember) {
        ...
        setGone((prev) =>
          prev && prev.name === urlMember && prev.shown === ""
            ? prev
            : { name: urlMember, shown: "" },
        );
    ...
    const target = resolveDefaultMember(
      safeGetItem(LAST_MEMBER_KEY),
      orderedMembers,
    );
    if (!target) return;
    if (urlMember) {
    ...
    <div className="flex h-full min-h-0 pb-2" data-testid="members-page">
      ...
      <div className="flex flex-1 min-w-0 gap-2 pr-2">
        ...
        >
          ...
          <div
            className="px-4 pb-2 text-[11px] text-muted"
          ...
          >
            ...
            {loadError
              ...
              : starredOnly || sourceFilter !== "all"
                ...
                : t("pages.membersPage.member_count", {
                    count: members.length,
                  })}
          </div>
          ...
          <div
            className="px-2 pb-2 flex flex-wrap items-center gap-1"
          ...
          >
            ...
            >
              ...
              {starredCount > 0 && (
                <span className="opacity-70">{starredCount}</span>
              )}
            </button>
          ...
          {gone && gone.shown === "" && (
            /* Below md a stale link lands on the roster; this is where the
             ...
             thread-side notice. */
            <div
              className="px-4 py-1.5 text-[13px] text-warn"
              role="status"
              data-testid="member-gone-roster-notice"
            >
          ...
          >
            ...
            {filteredOut && (
              <li
                className="px-4 py-6 text-xs text-muted"
                data-testid="member-filtered-out"
              >
            ...
            {sortedMembers.map((m) => (
              <li key={m.name} className="group/row relative">
                ...
                <button
                  ...
                  ref={
                    m.name === activeName ? scrollActiveRowIntoView : undefined
                  }
                  ...
                  <span className="relative shrink-0">
                    ...
                    <AnimatePresence initial={false}>
                      {(() => {
                        ...
                        const cycle =
                          lp.max_cycles > 0
                            ? t("pages.membersPage.patrol_cycles_of", {
                                n: lp.cycle_count,
                                max: lp.max_cycles,
                              })
                            : t("pages.membersPage.patrol_cycles_unlimited", {
                                n: lp.cycle_count,
                              });
                        const label = t("pages.membersPage.patrol_badge", {
                          cycle,
                        });
                        return (
                          <motion.span
                ...
                <button
                  ...
                  aria-label={t(
                    m.starred
                      ? "pages.membersPage.unstar"
                      : "pages.membersPage.star",
                    { name: m.name },
                  )}
                  title={t(
                    m.starred
          ...
          {active && (
            <>
              ...
              <header
                ...
                data-testid="member-thread-header"
              >
                <button
                  ...
                  onClick={() => {
                    if (
                      (location.state as { fromRoster?: boolean } | null)
                        ?.fromRoster
                    )
                      navigate(-1);
                ...
                <div
                  className="group/title min-w-0 flex-1 flex items-center gap-1.5"
                  data-testid="member-title-row"
                >
              ...
              {gone && gone.shown === active.name && (
                ...
                <div
                  className="px-4 py-2 text-[13px] text-warn"
                  role="status"
                  data-testid="member-gone-notice"
                >
                  {t("pages.membersPage.member_gone", {
                    name: gone.name,
                    shown: gone.shown,
                  })}
                </div>
              ...
              {activeCollision && (
                ...
                <div className="px-4 py-2">
                  <ErrorNotice
                    message={t("pages.membersPage.slug_collision", {
                      name: activeCollision,
                    })}
                    variant="inline"
                    askAgent
            ...
            const body = (
              ...
              DetailPanel header, so the body opens with the status alone. */}
                <div
                  ...
                  data-testid="member-drawer-status"
                >
                  {isRunning(active) ? (
                    <span className="text-ok">
                      {t("pages.membersPage.drawer_working")}
                    </span>
                  ) : active.last_active_ts ? (
                    <span className="text-muted">
                      {timeAgo(active.last_active_ts)}
                    </span>
                  ) : (
              ...
              spend) are deliberately absent rather than fabricated. */}
                <div
                  ...
                  data-testid="member-stats"
                >
                  <div className="border border-border rounded-lg px-3 py-2">
                    <div className="text-lg font-semibold leading-tight">
                      {activityLoading || activityError
                        ? "\u2013"
                        : `${todayCount}${todayIsFloor ? "+" : ""}`}
                    </div>
                    <div className="text-[11px] text-muted">
                      {t("pages.membersPage.stat_today")}
                    </div>
                  </div>
                  <div className="border border-border rounded-lg px-3 py-2">
                    <div className="text-lg font-semibold leading-tight">
                      {activityLoading || activityError
                        ? "\u2013"
                        : `${weekCount}${weekIsFloor ? "+" : ""}`}
                    </div>
                    <div className="text-[11px] text-muted">
                      {t("pages.membersPage.stat_week")}
                    </div>
                  </div>
                </div>
              ...
              needs input (info) > running (ok) > idle (muted). */}
                ...
                {drivingSessions.length === 0 && !slotsLoaded ? (
                  <div
                    ...
                    data-testid="member-driving-loading"
                    aria-hidden
                  >
                  ...
                  <div
                    ...
                    data-testid="member-driving-empty"
                  >
                  ...
                  <div className="mb-4">
                    <ul
                      ...
                      data-testid="member-driving-sessions"
                    >
                        ...
                        return (
                          <li key={s.key}>
                            <button
                              ...
                              onClick={() =>
                                navigate(
                                  `/chat?sid=${encodeURIComponent(s.key)}`,
                                )
                              }
                            ...
                            label is in the hover title and for AT). */}
                              {status.spoken ? (
                                <span
                                  className={`shrink-0 font-medium ${status.text}`}
                                >
                                  {label}
                                </span>
                              ) : (
                    ...
                    {drivingSessions.length > DRIVING_VISIBLE && (
                      <button
                        type="button"
                        onClick={() =>
                          setDrivingExpandedFor(
                            drivingExpanded ? "" : activeMemberKey,
                          )
                        }
                      ...
                      >
                        {drivingExpanded
                          ...
                          : t("pages.membersPage.driving_show_all", {
                              count: drivingSessions.length,
                            })}
                      </button>
                    )}
                  </div>
              ...
              a change, not a flicker. */}
                ...
                {!patrol.loaded ? (
                  <div
                    className="mb-4 space-y-1.5"
                    data-testid="member-patrol-loading"
                    aria-hidden
                  >
               ...
               safe here — a read failure on a drawer that holds no draft. */
                  ...
                  >
                    {patrolState === "active" && activePatrol ? (
                      <>
                        <div
                          className="text-[11px] font-medium text-accent mb-1.5"
                          data-testid="member-patrol-status"
                        >
                        ...
                        <dl className="text-[11px] space-y-1 m-0">
                          <div className="flex gap-2">
                            <dt className="w-24 shrink-0 text-muted">
                              {t("pages.membersPage.patrol_interval")}
                            </dt>
                            <dd
                              className="min-w-0 truncate m-0"
                              data-testid="member-patrol-interval"
                            >
                          ...
                          <div className="flex gap-2">
                            <dt className="w-24 shrink-0 text-muted">
                              {t("pages.membersPage.patrol_cycles")}
                            </dt>
                            <dd
                              className="min-w-0 truncate m-0"
                              data-testid="member-patrol-cycles"
                            >
                              {/* Self-describing here ("3 of 24"); the chip keeps its
                            compact "3/24", which alone read as a date. */}
                              {activePatrol.max_cycles > 0
                                ? t("pages.membersPage.patrol_cycles_of", {
                                    n: activePatrol.cycle_count,
                                    max: activePatrol.max_cycles,
                                  })
                                : t(
                                    "pages.membersPage.patrol_cycles_unlimited",
                                    { n: activePatrol.cycle_count },
                                  )}
                            </dd>
                          </div>
                          <div className="flex gap-2">
                            <dt className="w-24 shrink-0 text-muted">
                              {t("pages.membersPage.patrol_last_wake")}
                            </dt>
                            <dd
                              className="min-w-0 truncate m-0"
                              title={
                          ...
                          <div className="flex gap-2">
                            <dt className="w-24 shrink-0 text-muted">
                              {t("pages.membersPage.patrol_next_wake")}
                            </dt>
                            <dd
                              className="min-w-0 truncate m-0"
                              title={
                                activePatrol.next_due_ts > 0
                                  ? fmtDateTimeNumeric(activePatrol.next_due_ts)
                                  : undefined
                              }
                            ...
                            >
                              {(() => {
                                ...
                                switch (next.kind) {
                                  case "in":
                                    return t(
                                      "pages.membersPage.patrol_next_in",
                                      { time: next.time },
                                    );
                                  case "due":
                                    return t(
                                      "components.autoNudgePopover.next_cycle_due",
                                    );
                                  default:
                                    return t(
                                      "components.autoNudgePopover.next_cycle_unscheduled",
                                    );
                                }
                              })()}
                            </dd>
                          ...
                          {(activePatrol.banner || activePatrol.message) && (
                            <div className="flex gap-2">
                              <dt className="w-24 shrink-0 text-muted">
                                {t("pages.membersPage.patrol_instruction")}
                              </dt>
                      ...
                      >
                        ...
                        {activePatrol.stopped_reason && (
                          <span
                            ...
                            data-testid="member-patrol-reason"
                          >
                            {PATROL_STOPPED_REASON[activePatrol.stopped_reason]
                              ? t(
                                  PATROL_STOPPED_REASON[
                                    activePatrol.stopped_reason
                                  ],
                                )
                              : activePatrol.stopped_reason}
                        ...
                        {activePatrol.last_fire_ts > 0 && (
                          <span
                            className="block mt-0.5"
                            title={fmtDateTimeNumeric(
                              activePatrol.last_fire_ts,
                            )}
                          >
                            {t("pages.membersPage.patrol_last_wake_ago", {
                              when: timeAgo(activePatrol.last_fire_ts),
                            })}
                          </span>
                        )}
                      </div>
              ...
              render the affirmative "no recorded activity". */}
                {activityLoading ? (
                  <div
                    className="mb-4 space-y-1.5"
                    data-testid="member-activity-loading"
                    aria-hidden
                  >
                    <div className="h-3 rounded bg-accent/40 animate-pulse" />
                    <div className="h-3 w-3/4 rounded bg-accent/40 animate-pulse" />
                  ...
                  <ul
                    className="list-none m-0 p-0 mb-4 space-y-1.5"
                ...
                <div className="text-[11px] font-semibold tracking-wide text-muted mb-1.5 flex items-center">
                  <span className="flex-1">
                    {t("pages.membersPage.wake_sources")}
                  </span>
                ...
                {!wakeLoaded ? (
                  <div
                    className="mb-4 space-y-1.5"
                    data-testid="member-wake-loading"
                    aria-hidden
                  >
                    <div className="h-3 rounded bg-accent/40 animate-pulse" />
                  </div>
                  ...
                  <ul
                    className="list-none m-0 p-0 mb-4 space-y-1.5"
                    data-testid="member-wake-sources"
                  >
                  ...
                  "Last wake 6m ago" above "Nothing wakes this member". */}
                    {patrolState === "active" && activePatrol && (
                      ...
                      >
                        ...
                        <span className="text-muted shrink-0">
                          {t("pages.membersPage.wake_patrol_every", {
                            every: intervalText(activePatrol.idle_secs),
                          })}
                        </span>
                      </li>
                    )}
                    {wakeJobs.map((jb) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`min-w-0 truncate flex-1 ${jb.enabled ? "" : "text-muted"}`}
                        >
                          {jb.name}
                          {!jb.enabled &&
                        ...
                        <span
                          className="font-mono text-muted shrink-0 max-w-[45%] truncate"
                          title={jb.schedule}
                        >
                          {jb.schedule}
                        </span>
                    ...
                    {wakeHooks.map((tk) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`min-w-0 truncate flex-1 ${tk.enabled === false ? "text-muted" : ""}`}
                        >
                          {tk.label}
                          {tk.enabled === false &&
                            ` (${t("pages.membersPage.wake_paused")})`}
                        ...
                        <span className="text-muted shrink-0">
                          {t("pages.membersPage.wake_webhook")}
                        </span>
                      </li>
                    ))}
                  </ul>
                ...
                <dl className="text-xs space-y-2">
                  ...
                  <div className="flex gap-2">
                    <dt className="w-24 shrink-0 text-muted">
                      {t("pages.membersPage.model")}
                    </dt>
                    <dd className="min-w-0 truncate">
                      {active.model || t("pages.membersPage.inherited")}
                    </dd>
                  </div>
                  <div className="flex gap-2">
                    <dt className="w-24 shrink-0 text-muted">
