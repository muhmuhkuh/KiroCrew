...
describe('AgentBackendTab', () => {
  it('offers all four built-in backends', async () => {
    wrap()
  ...
  it('saves Pi through the same agent backend setting', async () => {
    wrap()
    fireEvent.click(await screen.findByRole('button', { name: 'Pi' }))
    await waitFor(() => expect(patchConfigMock).toHaveBeenCalledWith('agent.acp_backend', 'pi'))
  })
