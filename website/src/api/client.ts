...
function showSessionExpiredBanner(lead?: string): void {
 ...
 input.addEventListener("focus", () => {
  ...
  input.style.boxShadow =
   "0 0 0 3px rgba(255,255,255,0.25),0 0 20px rgba(255,255,255,0.1)";
 });
...
export function checkSessionExpired(r: Response): Response {
 if (
  r.status === 403 &&
  r.headers.get("X-Auth-Required") === "true" &&
  !_sessionExpiredShown
 ) {
...
const apiFailure = (r: Response, errText: string): ApiError => {
 ...
 const authRequired =
  r.status === 403 && r.headers.get("X-Auth-Required") === "true";
 ...
 return new ApiError(
  ...
  message,
  errText,
  authRequired || staleOwnerSession,
 );
};
...
export const api = {
 ...
 tunnelStatus: () =>
  fetch("/api/tunnel/status").then(j) as Promise<TunnelStatus>,
 system: () => fetch("/api/system").then(j),
 sessionStorage: () =>
  get("/api/system/session-storage").then(j) as Promise<SessionStorageReport>,
 ...
 sessionInventory: () =>
  get("/api/system/session-storage/sessions").then(
   j,
  ) as Promise<SessionInventoryList>,
 ...
 sessionInventoryTrash: (uids: string[]) =>
  post("/api/system/session-storage/trash", { uids }).then(
   j,
  ) as Promise<SessionTrashResult>,
 ...
 telemetryContextTrace: (slot: string) =>
  fetch("/api/telemetry/context-trace?slot=" + encodeURIComponent(slot)).then(
   j,
  ),
 ...
 wakatimeExportDownload: async (
  ...
  const filename =
   ...
   `wakatime-hours-${start}-to-${end}.${format}`;
 ...
 repairKiroPrerequisiteSpecs: () =>
  post("/api/kiro-prerequisite/repair-specs").then(
   j,
  ) as Promise<KiroPrerequisiteStatus>,
 ...
 updateKiroPrerequisiteCli: () =>
  post("/api/kiro-prerequisite/update-cli").then(
   j,
  ) as Promise<KiroPrerequisiteStatus>,
 ...
 kasLoginBeginLoopback: (provider: string) =>
  post("/api/kas-login/loopback", { provider }).then(
   j,
 ...
 kasLoginCancel: (login_id: string) =>
  post("/api/kas-login/cancel", { login_id }).then(j) as Promise<{
   ok: boolean;
  }>,
 kasLoginLogout: (identity: string) =>
  post("/api/kas-login/logout", { identity }).then(
   j,
  ) as Promise<KasLoginStatus>,
 onboardingImportScan: () =>
  get("/api/onboarding/import/scan").then(
   j,
  ) as Promise<AgentImportScanResponse>,
 onboardingImportApply: (body: AgentImportApplyRequest) =>
  post("/api/onboarding/import/apply", body).then(
   j,
  ) as Promise<AgentImportApplyResponse>,
 onboardingImportState: (body: { completed: true }) =>
  put("/api/onboarding/import/state", body).then(jNullable) as Promise<{
   ok?: boolean;
  } | null>,
 ...
 securityPosture: () =>
  get("/api/security/posture").then(j) as Promise<SecurityPostureData>,
 ...
 mobileConnectMethods: () =>
  get("/api/mobile-connect/methods").then(
   j,
  ) as Promise<MobileConnectMethodsData>,
 ...
 tailnetStatus: () =>
  get("/api/tailnet/status").then(j) as Promise<TailnetStatusData>,
 ...
 tailnetMobile: () =>
  get("/api/tailnet/mobile").then(j) as Promise<TailnetMobileData>,
 tailnetMobileConfigure: () =>
  post("/api/tailnet/mobile/configure", {}).then(
   j,
  ) as Promise<TailnetMobileConfigure>,
 tailnetMobilePublish: () =>
  post("/api/tailnet/mobile/publish", {}).then(
   j,
  ) as Promise<TailnetMobileMutation>,
 tailnetMobileUnpublish: () =>
  post("/api/tailnet/mobile/unpublish", {}).then(
   j,
  ) as Promise<TailnetMobileMutation>,
 ...
 tailnetMobileQr: (ttl?: string, sessionKey?: string) =>
  post("/api/tailnet/mobile/qr", ttl ? { ttl } : {}, sessionKey).then(
   j,
  ) as Promise<TailnetMobileQr>,
 ...
 deniedCommands: () =>
  get("/api/security/denied-commands").then(j) as Promise<DeniedCommandsData>,
 toggleBuiltinDeniedCommand: (id: string, enabled: boolean) =>
  patch("/api/security/denied-commands/builtins/" + encodeURIComponent(id), {
   enabled,
  }).then(j) as Promise<DeniedCommandsData>,
 ...
 toggleUserDeniedCommand: (id: string, enabled: boolean) =>
  patch("/api/security/denied-commands/user/" + encodeURIComponent(id), {
   enabled,
  }).then(j) as Promise<DeniedCommandsData>,
 ...
 listTrustedApps: () =>
  get("/api/security/trusted-apps").then(j) as Promise<TrustedAppsData>,
 ...
 setTrustAllApps: (value: boolean) =>
  put("/api/security/trusted-apps/allow-all", { value }).then(
   j,
  ) as Promise<TrustedAppsData>,
 ...
 governancePolicy: () =>
  get("/api/governance/policy").then(j) as Promise<GovernancePolicyData>,
 ...
 updateInstance: (id: string, body: Partial<AddInstanceBody>) =>
  patch("/api/instances/" + encodeURIComponent(id), body).then(
   j,
  ) as Promise<InstanceView>,
 removeInstance: (id: string) =>
  del("/api/instances/" + encodeURIComponent(id)).then(j),
 ...
 connectInstance: (
  ...
  post(
   "/api/instances/" +
    ...
    (opts?.rebuild
     ? "?rebuild=1"
     : opts?.onlyIfConnected
       ? "?only_if_connected=1"
       : ""),
  ).then(j) as Promise<InstanceTunnelStatus & { token?: string }>,
 refreshInstanceToken: (id: string) =>
  post("/api/instances/" + encodeURIComponent(id) + "/refresh-token").then(
   j,
  ) as Promise<InstanceTunnelStatus & { token?: string }>,
 disconnectInstance: (id: string) =>
  post("/api/instances/" + encodeURIComponent(id) + "/disconnect").then(
   j,
  ) as Promise<{
   disconnected: string;
   was_connected: boolean;
 ...
 restartInstance: (id: string) =>
  post("/api/instances/" + encodeURIComponent(id) + "/restart").then(
   j,
  ) as Promise<{
   ok: boolean;
   message: string;
 ...
 sendSessionToInstance: (id: string, slot: string) =>
  post("/api/instances/" + encodeURIComponent(id) + "/send-session", {
   slot,
  }).then(j) as Promise<{
   ok: boolean;
   instance: string;
 ...
 cloudPreflight: (profile?: string, region?: string) => {
  ...
  const s = p.toString();
  return get("/api/cloud/preflight" + (s ? "?" + s : "")).then(
   j,
  ) as Promise<CloudPreflight>;
 },
 cloudIamPolicy: () =>
  get("/api/cloud/iam-policy").then(j) as Promise<{ policy: string }>,
 cloudLaunches: () =>
  get("/api/cloud/launch").then(j) as Promise<{ jobs: LaunchJob[] }>,
 ...
 cloudLaunchStatus: (id: string) =>
  get("/api/cloud/launch/" + encodeURIComponent(id)).then(
   j,
  ) as Promise<LaunchJob>,
 cloudLaunchCancel: (id: string) =>
  post("/api/cloud/launch/" + encodeURIComponent(id) + "/cancel").then(
   j,
  ) as Promise<LaunchJob>,
 ...
 cloudLaunchSignin: (id: string) =>
  post("/api/cloud/launch/" + encodeURIComponent(id) + "/signin").then(
   j,
  ) as Promise<{ signin: CloudLaunchSignin }>,
 ...
 cloudStop: (tag: string, coords?: CloudCoords) =>
  post(
   "/api/cloud/" + encodeURIComponent(tag) + "/stop" + cloudQuery(coords),
  ).then(j) as Promise<{ ok?: boolean }>,
 cloudStart: (tag: string, coords?: CloudCoords) =>
  post(
   "/api/cloud/" + encodeURIComponent(tag) + "/start" + cloudQuery(coords),
  ).then(j) as Promise<{ ok?: boolean }>,
 cloudDestroy: (tag: string, coords?: CloudCoords) =>
  del("/api/cloud/" + encodeURIComponent(tag) + cloudQuery(coords)).then(
   j,
  ) as Promise<{
   ok?: boolean;
 ...
 saveMemoryPreferences: (content: string) =>
  put("/api/memory/preferences", { content }),
 memoryProjects: () => fetch("/api/memory/projects").then(j),
 saveMemoryProjects: (content: string) =>
  put("/api/memory/projects", { content }),
 memoryHistory: () => fetch("/api/memory/history").then(j),
 saveMemoryHistory: (content: string) =>
  put("/api/memory/history", { content }),
 memorySettings: () => fetch("/api/memory/settings").then(j),
 saveMemorySettings: (s: {
  history_idle_hours?: number;
  history_max_days?: number;
 }) => put("/api/memory/settings", s),
 // Vector memory
 vectorSemantic: () => fetch("/api/memory/semantic").then(j),
 vectorSemanticWrite: (key: string, value: string) =>
  put("/api/memory/semantic", { key, value, source: "user_explicit" }).then(j),
 vectorSemanticDelete: (key: string) =>
  del("/api/memory/semantic/" + encodeURIComponent(key)),
 vectorEpisodic: (limit = 50, offset = 0, tags?: string) =>
  fetch(
 ...
 vectorEpisodicSearch: (q: string, tags?: string) =>
  fetch(
   "/api/memory/episodic/search?q=" +
    encodeURIComponent(q) +
    (tags ? "&tags=" + encodeURIComponent(tags) : ""),
  ).then(j),
 vectorEpisodicDelete: (id: string) =>
  del("/api/memory/episodic/" + encodeURIComponent(id)),
 vectorStats: () => fetch("/api/memory/stats").then(j),
 vectorEvents: (limit = 50, offset = 0) =>
  fetch("/api/memory/events?limit=" + limit + "&offset=" + offset).then(j),
 ...
 agentDetail: (name: string) =>
  fetch("/api/agents/detail/" + encodeURIComponent(name)).then(j),
 ...
 agentResolvedModel: (agent: string) =>
  fetch("/api/agents/resolved-model?agent=" + encodeURIComponent(agent)).then(
   j,
  ),
 ...
 members: () =>
  fetch("/api/members").then(j) as Promise<{ members: MemberRosterRow[] }>,
 ...
 memberThread: (slug: string) =>
  post("/api/members/" + encodeURIComponent(slug) + "/thread").then(
   j,
  ) as Promise<{ slot_key: string; slug: string; member: string }>,
 ...
 effortLevels: (slot?: string) =>
  fetch(
   "/api/effort-levels" + (slot ? "?slot=" + encodeURIComponent(slot) : ""),
  ).then(j) as Promise<string[]>,
 ...
 chatSlotAgent: (slot: string, agent: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/agent", {
   agent,
  }).then(j) as Promise<{ ok?: boolean; agent?: string; workspace?: string }>,
 chatSlotModel: (slot: string, model: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/model", {
   model,
  }).then(j) as Promise<{ ok?: boolean; model?: string }>,
 /** This slot's auto-compact threshold override (null = follows the global). */
 chatSlotAutocompact: (slot: string) =>
  fetch("/api/chat/slots/" + encodeURIComponent(slot) + "/autocompact").then(
   j,
  ) as Promise<{
   pct: number | null;
   global_pct: number;
 ...
 setChatSlotAutocompact: (slot: string, pct: number | null) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/autocompact", {
   pct,
  }).then(j) as Promise<{
   ok?: boolean;
   pct: number | null;
 ...
 chatSlotWorkspace: (slot: string, workspace: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/workspace", {
   workspace,
  }).then(j),
 ...
 chatSlotReload: (slot: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/reload", {}).then(
   j,
  ) as Promise<{ ok?: boolean; error?: string }>,
 chatSlotProject: (slot: string, project: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/project", {
   project,
  }).then(j) as Promise<{ ok?: boolean; project?: string }>,
 ...
 recentProjects: () =>
  fetch("/api/recent-projects").then(j) as Promise<{ dirs: string[] }>,
 ...
 browseFiles: (path?: string) =>
  fetch(
   "/api/browse-files" + (path ? "?path=" + encodeURIComponent(path) : ""),
  ).then(j) as Promise<{
   path: string;
   parent: string;
 ...
 projectGit: (path: string) =>
  fetch("/api/project/git?path=" + encodeURIComponent(path)).then(
   j,
  ) as Promise<{
   path: string;
   repo: boolean;
 ...
 projectGitStatus: (path: string) =>
  fetch("/api/project/git/status?path=" + encodeURIComponent(path)).then(
   j,
  ) as Promise<{
   repo: boolean;
   repoRoot?: string;
 ...
 projectGitLog: (path: string, limit = 20) =>
  fetch(
   "/api/project/git/log?path=" + encodeURIComponent(path) + "&limit=" + limit,
  ).then(j) as Promise<{
   repo: boolean;
   commits: {
    sha: string;
 ...
 projectTree: (path: string) =>
  fetch("/api/project/tree?path=" + encodeURIComponent(path)).then(
   j,
  ) as Promise<{
   root: string;
   paths: string[];
 ...
 createCronFolder: (name: string) =>
  post("/api/cron-folders", { name }).then(j),
 ...
 updateHook: (id: string, body: object) =>
  put("/api/hooks/" + id, body).then(j),
 ...
 deleteWebhookToken: (id: string) =>
  del("/api/webhooks/tokens/" + encodeURIComponent(id)).then(j),
 ...
 setWebhooksEnabled: (enabled: boolean) =>
  post("/api/webhooks/switch", { enabled }).then(j),
 // Prompts (Agent SOPs)
 ...
 skills: (sessionKey?: string, agent?: string, signal?: AbortSignal) =>
  withDeadline(SKILLS_TIMEOUT_MS, signal, (s) =>
   get(
    "/api/skills" + (agent ? "?agent=" + encodeURIComponent(agent) : ""),
    sessionKey,
    s,
   ).then(j),
  ),
 /** Project-skills trust: this chat's grant state plus every stored grant. */
 skillTrust: (sessionKey?: string) =>
  get("/api/skills/-/trust", sessionKey).then(j),
 ...
 grantSkillTrust: (sessionKey: string | undefined, expectedKey: string) =>
  post("/api/skills/-/trust", { expected_key: expectedKey }, sessionKey).then(
   j,
  ),
 ...
 skill: (name: string) =>
  fetch(
   "/api/skills/" + name.split("/").map(encodeURIComponent).join("/"),
  ).then(j),
 ...
 createSkill: (name: string, content: string) =>
  post("/api/skills", { name, content }).then(j),
 updateSkill: (name: string, content: string) =>
  put("/api/skills/" + name.split("/").map(encodeURIComponent).join("/"), {
   content,
  }).then(j),
 deleteSkill: (name: string) =>
  del("/api/skills/" + name.split("/").map(encodeURIComponent).join("/")).then(
   j,
  ),
 ...
 approvePendingSkill: (slug: string) =>
  post(
   "/api/skills/-/pending/" + encodeURIComponent(slug) + "/approve",
   {},
  ).then(j),
 dismissPendingSkill: (slug: string) =>
  post(
   "/api/skills/-/pending/" + encodeURIComponent(slug) + "/dismiss",
   {},
  ).then(j),
 dismissAllPendingSkills: (slugs: string[]) =>
  post("/api/skills/-/pending/-/dismiss-all", { slugs }).then(j),
 pinSkill: (name: string, pinned: boolean) =>
  post("/api/skills/-/pin", { name, pinned }).then(j),
 ...
 skillsBudget: () =>
  get("/api/skills/-/budget").then(j) as Promise<
   import("../types").SkillBudgetResponse
  >,
 ...
 mcpCustomAdd: (
  servers: Record<string, import("../types").McpCustomSpec>,
  enable: boolean,
 ) =>
 ...
 mcpCustomUpdate: (name: string, spec: import("../types").McpCustomSpec) =>
  put(`/api/mcp/custom/${encodeURIComponent(name)}`, { spec }).then(
   j,
  ) as Promise<{ ok: boolean; name: string }>,
 mcpActive: (agent?: string) =>
  fetch(
   "/api/mcp/active" + (agent ? `?agent=${encodeURIComponent(agent)}` : ""),
  ).then(j),
 ...
 connectionsPremint: () =>
  post("/api/connections/premint").then(j) as Promise<{
   ok: boolean;
   preminting: string[];
  }>,
 ...
 connectionsTest: (slug: string) =>
  post("/api/connections/test", { slug }).then(
   j,
 ...
 connectionsCancel: (slug: string, token?: string) =>
  post("/api/connections/cancel", token ? { slug, token } : { slug }).then(
   j,
  ) as Promise<{ ok: boolean; slug: string; dropped: boolean }>,
 ...
 mcpGatewayServers: () =>
  fetch("/api/mcp-gateway/servers").then(j) as Promise<{
 ...
 mcpMeasureStart: () =>
  post("/api/mcp/measure", {}).then(j) as Promise<McpMeasureProgress>,
 mcpMeasureProgress: () =>
  fetch("/api/mcp/measure").then(j) as Promise<McpMeasureProgress>,
 ...
 mcpGatewaySetStubMany: (
  names: string[],
  stub: boolean,
  resolveEligibility?: boolean,
 ) =>
  post(
   "/api/mcp-gateway/servers/stub",
   resolveEligibility
    ? { names, stub, resolve_eligibility: true }
    : { names, stub },
  ).then(j) as Promise<{
   ok: boolean;
   names: string[];
 ...
 saveAgentConfig: (config: object) =>
  put("/api/agent/config", { config }).then(j),
 defaultAgent: () => fetch("/api/config/default-agent").then(j),
 setDefaultAgent: (agent: string) =>
  put("/api/config/default-agent", { agent }).then(j),
 ...
 acpBackends: () =>
  fetch("/api/acp-backends").then(j) as Promise<{
 ...
 pullRequestSource: (url: string, refresh = false) =>
  post("/api/source/pull-request", { url, refresh }).then(
   j,
  ) as Promise<PullRequestSource>,
 pullRequestChecks: (url: string) =>
  post("/api/source/pull-request/checks", { url }).then(j) as Promise<{
   checks: PullRequestCheck[];
  }>,
 pullRequestStatuses: (urls: string[]) =>
  post("/api/source/pull-request/status", { urls }).then(
   j,
  ) as Promise<PullRequestStatusBatch>,
 resolvePullRequestThread: (url: string, threadId: string) =>
  post("/api/source/pull-request/resolve", { url, threadId }).then(
   j,
  ) as Promise<{ resolved: boolean }>,
 unresolvePullRequestThread: (url: string, threadId: string) =>
  post("/api/source/pull-request/unresolve", { url, threadId }).then(
   j,
  ) as Promise<{ resolved: boolean }>,
 ...
 replyToPullRequestThread: (url: string, threadId: string, body: string) =>
  post("/api/source/pull-request/reply", { url, threadId, body }).then(
   j,
  ) as Promise<{ posted: boolean }>,
 /** Top-level comment on the pull request conversation. */
 commentOnPullRequest: (url: string, body: string) =>
  post("/api/source/pull-request/comment", { url, body }).then(j) as Promise<{
   posted: boolean;
  }>,
 enablePullRequestAutoMerge: (url: string, confirmImmediateMerge = false) =>
  post("/api/source/pull-request/auto-merge", {
   url,
   confirmImmediateMerge,
  }).then(j) as Promise<{ autoMerge: boolean; mergeMethod: string }>,
 markPullRequestReady: (url: string) =>
  post("/api/source/pull-request/ready", { url }).then(j) as Promise<{
   ready: boolean;
  }>,
 ...
 submitPullRequestReview: (
  ...
  post("/api/source/pull-request/submit-review", {
   url,
   reviewId,
   event,
   contentDigest,
  }).then(j) as Promise<{ submitted: boolean; event: string }>,
 ...
 chatSlotSourceLinks: (
  ...
  fetch("/api/chat/slots/" + encodeURIComponent(slot) + "/source-links").then(
   j,
  ),
 chatSlotDetail: (
  slot: string,
  ...
  return fetch("/api/chat/slots/" + encodeURIComponent(slot) + "?" + p, {
   signal,
  }).then(j);
 },
 ...
 deleteChatSlot: (slot: string) =>
  del("/api/chat/slots/" + encodeURIComponent(slot)).then(j),
 cleanupSessions: (
 ...
 stopChatSlotForce: (slot: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/stop?force=true").then(
   j,
  ),
 ...
 reorderQueuedMessages: (slot: string, order: string[]) =>
  put("/api/chat/slots/" + encodeURIComponent(slot) + "/queue/order", {
   order,
  }).then(j),
 ...
 endWait: (slot: string, waitId: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/end-wait", {
   wait_id: waitId,
  }).then(j),
 approveChatSlot: (
  slot: string,
  ...
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/approve", {
   action,
   ...extra,
  }).then(j),
 planAction: (slot: string, action: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/plan-action", {
   action,
  }).then(j),
 ...
 sideOpen: (slot: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/side/open", {}).then(
   j,
  ) as Promise<{
   ok: boolean;
   open: boolean;
 ...
 sideClose: (slot: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/side/close", {}).then(
   j,
  ) as Promise<{ ok: boolean; was_open: boolean }>,
 chatMode: (mode: string, slot?: string) =>
  post("/api/chat/mode", { mode, slot: slot || "" }).then(j),
 generateTitle: (slot: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/generate-title").then(
   j,
  ),
 resolveNavLinks: (links: { url: string; context: string }[]) =>
  post("/api/chat/nav/resolve-links", { links }).then(j) as Promise<{
   summaries: string[];
  }>,
 renameSlot: (slot: string, title: string) =>
  patch("/api/chat/slots/" + encodeURIComponent(slot) + "/title", {
   title,
  }).then(j),
 ...
 switchVariant: (slot: string, index: number) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/switch-variant", {
   index,
  }).then(j),
 editResend: (slot: string, ts: string, content: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/edit-resend", {
   ts,
   content,
  }).then(j),
 rewind: (slot: string, ts: string, content: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/rewind", {
   ts,
   content,
  }).then(j),
 ...
 pauseSlack: (slot: string, paused: boolean) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/slack-pause", {
   paused,
  }).then(j),
 ...
 pauseMirror: (slot: string, paused: boolean, origin = false) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/mirror-pause", {
   paused,
   origin,
  }).then(j),
 ...
 unlinkMirror: (slot: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/mirror-unlink").then(
   j,
  ),
 ...
 deleteChatFolder: (id: string) =>
  del("/api/chat/folders/" + encodeURIComponent(id)).then(j),
 ...
 setSlotPin: (slot: string, pinned: boolean) =>
  patch("/api/chat/slots/" + encodeURIComponent(slot) + "/pin", {
   pinned,
  }).then(j),
 setSlotMode: (slot: string, mode: string) =>
  patch("/api/chat/slots/" + encodeURIComponent(slot) + "/mode", { mode }).then(
   j,
  ),
 // Tags
 chatTags: () => fetch("/api/chat/tags", { headers: { ..._sk } }).then(j),
 createChatTag: (name: string, color?: string, status?: boolean) =>
  post("/api/chat/tags", { name, color: color || "", status: !!status }).then(
   j,
  ),
 updateChatTag: (
  id: string,
 ...
 deleteChatTag: (id: string) =>
  del("/api/chat/tags/" + encodeURIComponent(id)).then(j),
 setSlotTags: (slot: string, tags: string[]) =>
 ...
 dropSlotToColumn: (slot: string, columnId: string) =>
  post("/api/chat/slots/" + encodeURIComponent(slot) + "/drop", {
   column_id: columnId,
  }).then(j),
 tagColumns: () =>
  fetch("/api/chat/tag-columns", { headers: { ..._sk } }).then(j),
 ...
 deleteTagColumn: (id: string) =>
  del("/api/chat/tag-columns/" + encodeURIComponent(id)).then(j),
 ...
 ackNotification: (ts: string) =>
  post("/api/notifications/ack", { ts }).then(j),
 unackNotification: (ts: string) =>
  post("/api/notifications/unack", { ts }).then(j),
 ...
 // Handoff
 handoffChannels: () =>
  fetch("/api/handoff-channels").then(j) as Promise<Record<
   string,
   string
  > | null>,
 handoffSlot: (slot: string, channel?: string) =>
  post(
   "/api/chat/slots/" + encodeURIComponent(slot) + "/handoff",
 ...
 instancesSearchSessions: (q: string, limit = 50) =>
  fetch(
   "/api/instances/search-sessions?q=" +
    encodeURIComponent(q) +
    "&limit=" +
    limit,
  ).then(j),
 ...
 instancesCapabilities: (instanceId: string) =>
  fetch(
   "/api/instances/" + encodeURIComponent(instanceId) + "/capabilities",
  ).then(j) as Promise<RemoteCrewCapabilities>,
 sessionDetail: (key: string) =>
  fetch("/api/sessions/" + encodeURIComponent(key)).then(j),
 deleteSession: (key: string) =>
  del("/api/sessions/" + encodeURIComponent(key)).then(j),
 ...
 spawnDelete: (id: string) =>
  del("/api/spawn/" + encodeURIComponent(id)).then(j),
 spawnStopAll: (slot: string) => post("/api/spawn/stop-all", { slot }).then(j),
 spawnRetry: (id: string) =>
  post("/api/spawn/" + encodeURIComponent(id) + "/retry", {}).then(j),
 ...
 cancelTaskRunner: (taskId?: string) =>
  post("/api/taskrunner/cancel", taskId ? { task_id: taskId } : undefined).then(
   j,
  ),
 pauseTaskRun: (taskId: string) =>
  post("/api/taskrunner/" + encodeURIComponent(taskId) + "/pause").then(j),
 deleteTaskRun: (taskId: string) =>
  del("/api/taskrunner/" + encodeURIComponent(taskId)).then(j),
 retryTaskRun: (taskId: string, fromStep: number) =>
  post("/api/taskrunner/" + encodeURIComponent(taskId) + "/retry", {
   from_step: fromStep,
  }).then(j),
  ...
  */
 workflowRuns: () =>
  get("/api/workflows/runs").then(j) as Promise<{
 ...
 workflowDefinitions: (search = "") =>
  ...
  ).then(j) as Promise<{ definitions: WorkflowDefinition[] }>,
 ...
 promoteWorkflowRun: (
  ...
  post(`/api/workflows/runs/${encodeURIComponent(runId)}/promote`, body).then(
   j,
  ) as Promise<{
   ok: boolean;
   definition: WorkflowDefinition;
 ...
 updateWorkflowDefinition: (
  workflowRef: string,
  body: Omit<WorkflowDefinitionWrite, "derived_from"> & {
   expected_revision: number;
  },
 ) =>
  patch(
   `/api/workflows/definitions/${encodeURIComponent(workflowRef)}`,
   body,
  ).then(j) as Promise<{ ok: boolean; definition: WorkflowDefinition }>,
 runWorkflowDefinition: (
  workflowRef: string,
  input: string,
  args: Record<string, unknown> = {},
 ) =>
  post(`/api/workflows/definitions/${encodeURIComponent(workflowRef)}/run`, {
   input,
   args,
  }).then(j) as Promise<{
   run_id: string;
 ...
 refineTaskInput: (input: string) =>
  post("/api/taskrunner/refine", { input }).then(j),
 ...
 updatePlan: (taskId: string, steps: PlanStepInput[]) =>
  put("/api/taskrunner/" + encodeURIComponent(taskId) + "/plan", {
   steps,
  }).then(j),
 ...
 planContext: (taskId: string) =>
  fetch("/api/taskrunner/" + encodeURIComponent(taskId) + "/plan-context").then(
   j,
  ),
 ...
 exportPlanYaml: async (taskId: string) => {
  const r = await get(
   "/api/taskrunner/" + encodeURIComponent(taskId) + "/plan.yaml",
  );
  if (!r.ok) {
   const t = await r.text();
   throw new ApiError(r.status, t || `HTTP ${r.status}`);
 ...
 setUpdateChannel: (channel: string) =>
  post("/api/update/channel", { channel }).then(
   j,
 ...
 fileSearch: (
  ...
  if (limit) p.set("limit", String(limit));
  return fetch(`/api/file-search?${p}`, signal ? { signal } : undefined).then(
   j,
  ) as Promise<{
   results: Array<{
    path: string;
 ...
 uploadFiles: async (files: File[]) => {
  ...
  const resized = prepared
   .map((p) => p.info)
   .filter((i): i is ResizeInfo => i !== null);
  const fd = new FormData();
  ...
  return {
   ...(body as { paths: string[]; error?: string }),
   resized,
   resizedByPath,
  };
 },
 ...
 featureVideoNext: (sessionKey?: string) =>
  get("/api/feature-videos/next", sessionKey).then(
   j,
  ) as Promise<FeatureVideoNext>,
 ...
 featureVideoFeedback: (
  id: string,
  status: "seen" | "dismissed",
  sessionKey?: string,
 ) =>
  post("/api/feature-videos/feedback", { id, status }, sessionKey).then(
   j,
  ) as Promise<{ ok: true }>,
 updateDashboardConfig: (body: object) =>
  put("/api/dashboard/config", body).then(j),
 createTheme: (body: object) => post("/api/themes", body).then(j),
 installTheme: (
  source: { type: "local"; path: string } | { type: "github"; url: string },
 ...
 deleteTheme: (slug: string) =>
  del("/api/themes/" + encodeURIComponent(slug)).then(j),
 themeDetail: (slug: string) =>
  fetch("/api/themes/" + encodeURIComponent(slug)).then(j),
 ...
 grantAwsConsent: (
  service: string,
  shown: { profile: string; region: string; account: string },
 ) =>
 ...
 revokeAwsConsent: (service: string) =>
  del("/api/aws/consent?service=" + encodeURIComponent(service)).then(
   j,
 ...
 voiceSynthesize: (
  ...
  window.dispatchEvent(
   new CustomEvent("voice-synthesis-start", { detail: { slot, request_id } }),
  );
  return post("/api/voice/synthesize", { slot, text, ...opts, request_id })
   .then(j)
   .catch((error) => {
    const code =
     error instanceof ApiError ? parseErrorCode(error.body) : undefined;
    window.dispatchEvent(
 ...
 channelGet: (id: string) =>
  fetch("/api/channels/" + encodeURIComponent(id)).then(j),
 channelCreate: (topic: string, agents: object[]) =>
  post("/api/channels", { topic, agents }).then(j),
 channelClose: (id: string) =>
  del("/api/channels/" + encodeURIComponent(id)).then(j),
 channelPost: (
  id: string,
  content: string,
 ...
 enableApp: (name: string) =>
  post("/api/apps/" + encodeURIComponent(name) + "/enable").then(j),
 disableApp: (name: string) =>
  post("/api/apps/" + encodeURIComponent(name) + "/disable").then(j),
 openApp: (name: string) =>
  post("/api/apps/" + encodeURIComponent(name) + "/open").then(j),
 uninstallApp: (
  name: string,
  keepData = true,
 ...
 uninstallPreview: (name: string) =>
  fetch("/api/apps/" + encodeURIComponent(name) + "/uninstall/preview").then(
   j,
  ) as Promise<{
   app: string;
 ...
 updateApp: (name: string, source?: string) =>
  post(
   "/api/apps/" + encodeURIComponent(name) + "/update",
   source ? { source } : {},
  ).then(j),
 ...
 listRegistries: () =>
  fetch("/api/apps/registries").then(j) as Promise<{
   ...
   pinned?: { name: string; repo: string; branch: string; trust?: string }[];
  }>,
 updateRegistries: (
  registries: { name: string; repo: string; branch: string; trust?: string }[],
 ) =>
 ...
 refreshAppStore: () =>
  post("/api/app-store/refresh").then(j) as Promise<{ ok: boolean }>,
 refreshRegistries: (repo?: string) =>
  post("/api/apps/registries/refresh", repo ? { repo } : {}).then(
   j,
  ) as Promise<{
   ok: boolean;
   refreshed: string[];
 ...
 installFromRegistry: (name: string) =>
  post("/api/apps/registry/install", { name }).then(j),
 /**
 ...
 installFromRegistryStream: async (
  ...
  try {
   ...
   return {
    ok: false,
    error: i18nT("api.client.stream_ended_without_completion"),
   };
  } finally {
   reader.releaseLock();
  }
 ...
 artifact: (slug: string) =>
  get(`/api/artifacts/${encodeURIComponent(slug)}`).then(j),
 ...
 createArtifact: (
  ...
  post(
   ...
   body,
   sessionKey ??
    (body.origin_session_key
     ? `dashboard:${body.origin_session_key}`
     : undefined),
  ).then(j),
 ...
 settleBlankArtifact: (
  ...
  post(`/api/artifacts/${encodeURIComponent(slug)}/settle`, body).then(
   j,
  ) as Promise<{ outcome: "kept" | "saved" | "deleted" }>,
 updateArtifact: (
  slug: string,
  body: {
   content?: string;
 ...
 deleteArtifact: (slug: string) =>
  del(`/api/artifacts/${encodeURIComponent(slug)}`).then(j),
 // Artifact library folders
 artifactFolders: () => get("/api/artifact-folders").then(j),
 createArtifactFolder: (body: {
  name: string;
  parent_id?: string;
  color?: string;
 ...
 setArtifactFolder: (slug: string, folderId: string) =>
  patch(`/api/artifacts/${encodeURIComponent(slug)}/folder`, {
   folder_id: folderId,
  }).then(j),
 ...
 getArtifactPublishProviders: (
  ...
  get(`/api/artifacts/publish-providers?kind=${encodeURIComponent(kind)}`).then(
   j,
  ),
 ...
 reprobeArtifactNotice: (slug: string) =>
  post(
   `/api/artifacts/${encodeURIComponent(slug)}/publish/reprobe-notice`,
   {},
  ).then(j),
 ...
 refreshArtifactSharing: (slug: string) =>
  post(`/api/artifacts/${encodeURIComponent(slug)}/publish/refresh`, {}).then(
   j,
  ),
 ...
 overwriteRemote: (slug: string) =>
  post(`/api/artifacts/${encodeURIComponent(slug)}/overwrite-remote`, {}).then(
   j,
  ),
 ...
 getBrowserInstall: () =>
  get("/api/browser/install").then(j) as Promise<BrowserInstallData>,
 setBrowserToken: (token: string) =>
  put("/api/browser/token", { token }).then(j) as Promise<{
   ok: boolean;
   token: boolean;
  }>,
 installBrowserCli: () =>
  post("/api/browser/install", {}).then(j) as Promise<BrowserInstallData>,
 installBrowserEngine: (engine: string) =>
  post("/api/browser/engine", { engine }).then(
   j,
  ) as Promise<BrowserInstallData>,
 getBrowserView: () =>
  get("/api/browser/view").then(j) as Promise<BrowserViewData>,
 startBrowserView: () =>
  post("/api/browser/view/start", {}).then(j) as Promise<BrowserViewData>,
 ...
 saveComputerUseConfig: (body: Partial<ComputerUseConfigSave>) =>
  put("/api/computer-use/config", body).then(
   j,
  ) as Promise<ComputerUseConfigData>,
 // Slack integration config
 getSlackConfig: () =>
  get("/api/slack/config").then(j) as Promise<SlackConfigData>,
 getSlackManifest: () =>
 ...
 getDiscordConfig: () =>
  get("/api/discord/config").then(j) as Promise<DiscordConfigData>,
 ...
 getTelegramConfig: () =>
  get("/api/telegram/config").then(j) as Promise<TelegramConfigData>,
 ...
 getWeComConfig: () =>
  get("/api/wecom/config").then(j) as Promise<WeComConfigData>,
 getFeishuConfig: () =>
  get("/api/feishu/config").then(j) as Promise<FeishuConfigData>,
 ...
 getWebexConfig: () =>
  get("/api/webex/config").then(j) as Promise<WebexConfigData>,
 ...
 getIMessageConfig: () =>
  get("/api/imessage/config").then(j) as Promise<IMessageConfigData>,
 ...
 getGovernanceChannels: () =>
  get("/api/governance/channels").then(j) as Promise<
   Record<string, boolean | null>
  >,
 getTeamsConfig: () =>
  get("/api/teams/config").then(j) as Promise<TeamsConfigData>,
 ...
 getWeixinConfig: () =>
  get("/api/weixin/config").then(j) as Promise<WeixinConfigData>,
 saveWeixinConfig: (body: Partial<WeixinConfigSave>) =>
  put("/api/weixin/config", body).then(j) as Promise<{
   ok: boolean;
   restart_required: boolean;
  }>,
 weixinQrStart: () =>
 ...
 weixinQrStatus: (sessionId: string) =>
  get(
   `/api/channels/weixin/qr/status?session_id=${encodeURIComponent(sessionId)}`,
  ).then(j) as Promise<{
   status: string;
   connected?: boolean;
   account_id?: string;
 ...
 getWhatsAppConfig: () =>
  get("/api/whatsapp/config").then(j) as Promise<WhatsAppConfigData>,
 ...
 getWhatsAppGroups: () =>
  get("/api/whatsapp/groups").then(j) as Promise<{
   groups: { jid: string; name: string }[];
  }>,

 // Auto-research
 researchValidate: (body: object) =>
  post("/api/apps/auto-research/validate", body).then(j),
 researchGrillExpand: (body: object) =>
  post("/api/apps/auto-research/grill/expand", body).then(j),
 /**
 ...
 researchCampaign: (id: string) =>
  get("/api/apps/auto-research/campaigns/" + id).then(j),
 researchCreate: (body: object) =>
  post("/api/apps/auto-research/campaigns", body).then(j),
 ...
 researchAddQuestion: (id: string, text: string) =>
  post("/api/apps/auto-research/campaigns/" + id + "/questions", { text }).then(
   j,
  ),
 ...
 researchDelete: (id: string) =>
  del("/api/apps/auto-research/campaigns/" + id).then(j),
 ...
 publishProviders: () =>
  get("/api/publish-providers").then(j) as Promise<{
   providers: AppPublishProvider[];
  }>,
