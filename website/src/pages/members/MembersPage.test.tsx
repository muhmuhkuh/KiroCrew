...
vi.mock("../../components/ChatPane", () => ({
  default: ({
    ...
    <div
      data-testid="chat-pane-stub"
      data-agent-locked={agentLocked ? "1" : "0"}
      data-follow-content-width={followContentWidth ? "1" : "0"}
    >
      {slotKey}
    </div>
  ),
...
async function renderPage(
  ...
  defaultAgent = "kirocrew",
  {
    route = "/members",
    thread,
  }: { route?: string; thread?: Record<string, unknown> | Error } = {},
) {
...
beforeEach(() => {
  ...
  vi.mocked(api.webhooks).mockImplementation(() =>
    Promise.resolve({ tokens: [] }),
  );
  vi.mocked(api.defaultAgent).mockImplementation(() =>
    Promise.resolve({ default_agent: "" }),
  );
  ...
  vi.mocked(api.autonudgeList).mockImplementation(() =>
    Promise.resolve({ enabled: true, loops: [] }),
  );
...
describe("MembersPage roster", () => {
  ...
  it("shows the load-failure state when the roster call rejects", async () => {
    ...
    expect(screen.getByTestId("member-count")).not.toHaveTextContent(
      /members/i,
    );
  });
});
...
describe("MembersPage roster cache (React Query)", () => {
  ...
  it("a second mount mounts the cached thread at once; the repair POST is re-issued but never waited on", async () => {
    ...
    expect(await screen.findByTestId("chat-pane-stub")).toHaveTextContent(
      "member-oncall",
    );
    ...
    (api.memberThread as ReturnType<typeof vi.fn>).mockReturnValue(
      new Promise(() => {}),
    );
    utils.rerender(page);
    expect(await screen.findByTestId("chat-pane-stub")).toHaveTextContent(
  ...
  it("a failed repair over a cached thread keeps the thread up and says the RECONNECT failed, not the open", async () => {
    ...
    utils.rerender(<LocationProbe />);
    (api.memberThread as ReturnType<typeof vi.fn>).mockRejectedValue(
      new Error("boom"),
    );
    utils.rerender(page);
    ...
    expect(screen.getByTestId("chat-pane-stub")).toHaveTextContent(
      "member-oncall",
    );
  });
...
describe("MembersPage thread", () => {
  ...
  it("orders the roster by most recent activity, never-talked members last alphabetically", async () => {
    ...
    const names = Array.from(
      list.querySelectorAll("li button .font-medium"),
  ...
  it("opens a bound member through the thread endpoint too — the roster binding is never mounted unverified", async () => {
    ...
    ).toHaveTextContent("member-oncall");
  });
  ...
  it("surfaces a slug collision instead of silently mounting another member thread", async () => {
    ...
    fireEvent.click(await rosterRow("oncall"));
    expect(
      await screen.findByText(/shares its short name with/i),
    ).toBeInTheDocument();
  ...
  it("keeps a late failure of a previously selected member out of the active view", async () => {
    ...
    expect(
      await screen.findByTestId("chat-pane-stub", PANE_READY),
    ).toHaveTextContent("member-alpha");
    ...
    await waitFor(() =>
      expect(
        screen.queryByText(/Could not open this member's conversation/i),
      ).toBeNull(),
    );
    expect(screen.getByTestId("chat-pane-stub")).toHaveTextContent(
      "member-beta",
...
describe("MembersPage drawer and edit jump", () => {
  it("shows the read-only config summary and the shared-memory disclosure", async () => {
    await renderPage([
      row({ bound: true, slot_key: "member-oncall", model: "claude-opus-5" }),
    ]);
    fireEvent.click(await rosterRow("oncall"));
  ...
  it("the drawer is hosted in the shared DetailPanel: drag-resize handle present, header close works", async () => {
    ...
    expect(
      screen.getByRole("separator", { name: /resize/i }),
    ).toBeInTheDocument();
  ...
  it("the edit affordance lives in the drawer only and navigates to this member's editor in the crew manager", async () => {
    ...
    for (const btn of screen.getAllByRole("button", {
      name: /edit in crew manager/i,
    })) {
      fireEvent.click(btn);
      expect(navigateSpy).toHaveBeenCalledWith(
        "/capabilities?tab=crews&crew=oncall",
      );
  ...
  it("the roster header has an add-member entry that lands on the crew manager's create form", async () => {
    ...
    expect(navigateSpy).toHaveBeenCalledWith(
      "/capabilities?tab=crews&new=1&from=members",
    );
  });
...
describe("MembersPage unread drain", () => {
  ...
  it("opening a flagged member thread drains its unread flag", async () => {
    ...
    await screen.findByTestId("chat-pane-stub", PANE_READY);
    await waitFor(() =>
      expect(store.getState().dashboard.unreadSlots).not.toContain(
        "member-oncall",
      ),
    );
  });
  ...
  it("a flagged member shows the unread dot on its roster row; unflagged members do not", async () => {
    ...
    expect(
      await screen.findByTestId("chat-pane-stub", PANE_READY),
    ).toHaveTextContent("member-scout");
...
describe("MembersPage drawer — driving sessions", () => {
  ...
  it('before the first slots frame it shows a skeleton, never the affirmative "not driving"', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("member-driving-empty")).toBeInTheDocument(),
    );
  ...
  it("renders the empty state when no live slot was created by the member", async () => {
    ...
    expect(screen.getByTestId("member-driving-empty")).toHaveTextContent(
      /not driving any sessions/i,
    );
  ...
  it("lists only the sessions this member created, newest activity first, with the sidebar status vocabulary", async () => {
    await openDrawer([
      ...
      worker("chat-1-running", {
        running: true,
        last_turn_ts: "2026-09-04T11:00:00Z",
      }),
      worker("chat-1-approval", {
        running: true,
      ...
      worker("chat-1-input", {
        needs_input: true,
        last_turn_ts: "2026-09-04T10:00:00Z",
      }),
      worker("chat-1-foreign", {
        created_by: "member-research",
        last_turn_ts: "2026-09-04T13:00:00Z",
      }),
    ]);
  ...
  it("a slots frame never reorders the list; a change to the driven set re-sorts it", async () => {
    ...
    act(() => {
      store.dispatch(
        sseSlots([
          ...
          worker("chat-1-b", {
            running: true,
            last_turn_ts: "2026-09-04T13:30:00Z",
          }),
        ] as never),
      );
    });
    await waitFor(() =>
      expect(screen.getAllByTestId("member-driving-row")[1]).toHaveAttribute(
        "data-status",
        "running",
      ),
    );
    ...
    await waitFor(() =>
      expect(screen.getAllByTestId("member-driving-row")).toHaveLength(3),
    );
  ...
  it("folds past five rows behind a Show-all toggle that expands and collapses", async () => {
    await openDrawer(
      Array.from({ length: 7 }, (_, i) => worker(`chat-1-w${i}`)),
    );
  ...
  it("a worker closing (leaving the live slots) drops out of the list live", async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByTestId("member-driving-row")).toHaveLength(1),
    );
  });
  ...
  it("the two parked states are spoken as visible text and every row carries a hover title", async () => {
    ...
    expect(approval).toHaveAttribute(
      "title",
      expect.stringMatching(/needs approval/i),
    );
  ...
  it("the fold is per member: expanding one member does not leak into the next drawer opened", async () => {
    ...
    (api.memberThread as ReturnType<typeof vi.fn>).mockImplementation(
      (slug: string) =>
        Promise.resolve({
          slot_key: `member-${slug}`,
          slug,
      ...
      utils.store.dispatch(
        sseSlots([
          ...
          ...Array.from({ length: 6 }, (_, i) =>
            worker(`chat-1-r${i}`, { created_by: "member-research" }),
          ),
        ] as never),
      );
    });
    ...
    await waitFor(() =>
      expect(api.memberThread).toHaveBeenCalledWith("research"),
    );
    await waitFor(() =>
      expect(screen.getAllByTestId("member-driving-row")).toHaveLength(5),
    );
    expect(screen.getByTestId("member-driving-toggle")).toHaveAttribute(
      "aria-expanded",
      "false",
    );
  });
...
describe("MembersPage auto patrol (monitor loop status)", () => {
  ...
  async function openDrawerWith(registry: { loops: unknown[] }) {
    ...
    await screen.findByTestId("member-drawer");
    await waitFor(() =>
      expect(screen.queryByTestId("member-patrol-loading")).toBeNull(),
    );
    return utils;
  }
  ...
  it("an active loop renders as patrolling, with interval, cycles, last and next wake, and the banner-or-instruction line", async () => {
    ...
    expect(screen.getByTestId("member-patrol-status")).toHaveTextContent(
      /patrolling/i,
    );
    ...
    expect(screen.getByTestId("member-patrol-cycles")).toHaveTextContent(
      "3 of 24",
    );
    // Interval via the shared narrow-unit duration formatter (`20m`).
    expect(screen.getByTestId("member-patrol-interval")).toHaveTextContent(
      "20m",
    );
    ...
    expect(screen.getByTestId("member-patrol-next")).toHaveTextContent(
      /^Due in \d/,
    );
    expect(screen.getByTestId("member-patrol-next")).not.toHaveTextContent(
      /next cycle/i,
    );
  ...
  it('no loop on the member slot renders "no patrol scheduled" — and a loop on ANOTHER slot does not leak in', async () => {
    await openDrawerWith({
      loops: [
        loop({ slot_key: "member-research" }),
        loop({ slot_key: "chat-1-abc" }),
      ],
    });
    expect(screen.getByTestId("member-patrol")).toHaveAttribute(
      "data-state",
      "none",
    );
    expect(screen.getByTestId("member-patrol-status")).toHaveTextContent(
      /no patrol scheduled/i,
    );
  });
  ...
  it('a stopped loop keeps its reason visible instead of collapsing into "no patrol scheduled"', async () => {
    ...
    await openDrawerWith({
      loops: [loop({ active: false, stopped_reason: "cycle_cap" })],
    });
    expect(screen.getByTestId("member-patrol")).toHaveAttribute(
      "data-state",
      "stopped",
  ...
  it('an active patrol is listed under Wake sources, so the card cannot say "nothing wakes this member" above a live one', async () => {
    await openDrawerWith({ loops: [loop()] });
    await waitFor(() =>
      expect(screen.queryByTestId("member-wake-loading")).toBeNull(),
    );
    expect(screen.getByTestId("member-wake-patrol")).toHaveTextContent(
      /auto patrol/i,
    );
    expect(screen.getByTestId("member-wake-patrol")).toHaveTextContent(
      /every 20m/i,
  ...
  it("a failed registry read renders the error state, never the affirmative empty state", async () => {
    (api.autonudgeList as ReturnType<typeof vi.fn>).mockRejectedValue(
      new Error("boom"),
    );
    await renderPage([row({ bound: true, slot_key: "member-oncall" })]);
    fireEvent.click(await rosterRow("oncall"));
    ...
    expect(screen.getByTestId("member-roster-patrol-error")).toHaveAttribute(
      "role",
      "alert",
    );
  });
  ...
  it("a failed registry read is announced on the roster even with no drawer open", async () => {
    ...
    await rosterRow("oncall");
    expect(
      await screen.findByTestId("member-roster-patrol-error"),
    ).toHaveTextContent(/patrol status/i);
  ...
  it("a loop that stops in place (registry re-read flips active to false) drops the badge instead of recolouring it", async () => {
    ...
    await rosterRow("oncall");
    expect(await screen.findByTestId("member-patrol-dot")).toHaveAttribute(
      "data-state",
      "active",
    );
    (api.autonudgeList as ReturnType<typeof vi.fn>).mockResolvedValue({
...
describe("MembersPage member edit entry (issue #9425)", () => {
  ...
  it('the DM header carries a pencil right of the name, named "Edit member", that opens this member\'s editor', async () => {
    ...
    expect(
      titleRow.compareDocumentPosition(
        screen.getByTestId("member-drawer-toggle"),
    ...
    expect(navigateSpy).not.toHaveBeenCalledWith(
      expect.stringContaining("avatar=1"),
    );
  });
  ...
  it("the pencil is invisible at rest, revealed by hovering the title row or by focus, and low-contrast-persistent on touch", async () => {
    ...
    expect(screen.getByTestId("member-title-row").className).toContain(
      "group/title",
    );
...
describe("MembersPage default member, memory and URL", () => {
  ...
  it("a fresh visit opens the first member in display order — never the empty column", async () => {
    ...
    expect(
      await screen.findByTestId("chat-pane-stub", PANE_READY),
    ).toHaveTextContent("member-fresh-talker");
  ...
  it("a refresh-frame refetch never reorders the roster; a membership change re-sorts it", async () => {
    ...
    const names = () =>
      roster()
        ...
        .map(
          (li) => within(li).queryByText(/^(alpha|beta|gamma)$/)?.textContent,
        )
        .filter(Boolean);
    ...
    membersMock.mockResolvedValue({
      members: [
        ...
        row({
          ...
          slug: "beta",
          last_active_ts: 999,
          last_message: "fresh row content",
        }),
      ],
      default_agent: "kirocrew",
    });
    act(() => {
      void utils.queryClient.invalidateQueries({
  ...
  it("a URL naming a member wins over the remembered one (shallow link)", async () => {
    ...
    await renderPage(alphaBeta(), "kirocrew", {
      route: "/members?member=beta",
    });
    expect(
      await screen.findByTestId("chat-pane-stub", PANE_READY),
    ).toHaveTextContent("member-beta");
  ...
  it("a gone link falls back to the REMEMBERED member first, and leaves the memory alone", async () => {
    ...
    expect(screen.getByTestId("member-gone-notice")).toHaveTextContent(
      /^Showing beta/,
    );
    expect(currentUrl()).toBe("/members?member=beta");
    expect(localStorage.getItem(LAST_MEMBER_KEY)).toBe("beta");
  ...
  it("clicking a member writes the URL and the memory", async () => {
    ...
    expect(roster().getByText("beta").closest("button")).toHaveAttribute(
      "aria-current",
      "true",
    );
  });
  ...
  it("a link that outruns the cached roster waits for the refetch instead of calling the member gone", async () => {
    ...
    function Elsewhere() {
      ...
      return (
        <button
          data-testid="return-with-new-member"
          onClick={() => nav("/members?member=staging")}
        >
          go
        </button>
      );
  ...
  describe("below md", () => {
    ...
    afterEach(() => {
      if (ownDescriptor)
        Object.defineProperty(window, "matchMedia", ownDescriptor);
      else
