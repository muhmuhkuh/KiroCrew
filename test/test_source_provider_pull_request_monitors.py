...
def _probe(provider, target: str, **kwargs):
    previous = kwargs.pop("previous_observation", None)
    call_kwargs = dict(kwargs)
    if previous is not None:
        call_kwargs["previous_observations"] = {target: previous}
    results = provider.probe((target,), **call_kwargs)
...
def test_gitlab_failed_pipeline_beats_pending_mergeability():
    ...
    result = _probe(provider, "https://gitlab.com/acme/widgets/-/merge_requests/8")
...
def test_gitlab_revoked_host_is_a_terminal_authorization_failure(
    ...
    result = _probe(provider, "https://gitlab.example.com/acme/widgets/-/merge_requests/8")
...
def test_azure_requested_changes_and_active_thread_are_actionable():
    ...
    result = _probe(provider, "https://dev.azure.com/acme/project/_git/widgets/pullrequest/9")
...
def test_bitbucket_unresolved_task_is_actionable_and_payload_is_bounded():
    ...
    result = _probe(provider, "https://bitbucket.org/acme/widgets/pull-requests/10")
