/**
 * Screenshot harness for a merge conflict on the selected pull-request tab.
 *
 ...
 *
 * The selected conflict has passing CI while the background pull request is
 * clean with passing CI. The strip must replace only the selected tab's green
 * check with a red conflict glyph; the clean tab keeps its check. The header
 * blocker remains visible as the detailed explanation.
 *
...
const slots = [{
  ...
  source_links: [
    { provider: 'github', number: 6999, url: PR_URL, state: 'open', ci: 'passed', mergeable: 'conflicting', mergeStateStatus: 'dirty' },
    { provider: 'github', number: 6998, url: OTHER_URL, state: 'open', ci: 'passed', mergeable: 'mergeable', mergeStateStatus: 'clean' },
  ],
...
/** Full payload for the SELECTED pull request: merge pair settled, CI passed. */
const source = {
  provider: 'github',
  url: PR_URL,
  number: 6999,
  ...
  checks: [
    { name: 'Frontend Tests', workflow: 'CI', status: 'COMPLETED', conclusion: 'SUCCESS', bucket: 'passed', url: `${PR_URL}/checks`, startedAt: '', completedAt: '' },
  ],
  comments: [],
  files: [
 ...
 *  here. */
const statuses = {
  [PR_URL]: { state: 'open', ci: 'passed', mergeable: 'conflicting', mergeStateStatus: 'dirty' },
  [OTHER_URL]: { state: 'open', ci: 'passed', mergeable: 'mergeable', mergeStateStatus: 'clean' },
}
...
async function main() {
  ...
  const selectedTab = tablist.getByRole('tab', { name: /PR #6999/i })
  const cleanTab = tablist.getByRole('tab', { name: /PR #6998/i })
  await selectedTab.getByLabel('Merge conflicts').waitFor()
  if (await selectedTab.getByLabel('Checks passed').count()) {
    throw new Error('conflicted tab still renders Checks passed')
  }
  await cleanTab.getByLabel('Checks passed').waitFor()
  if (await cleanTab.getByLabel('Merge conflicts').count()) {
    throw new Error('clean tab incorrectly renders Merge conflicts')
  }
  console.log(`tablist:${tabs} strip:"${strip}" selected:conflict clean:passed`)
