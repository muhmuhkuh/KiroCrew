...
vi.mock("../apps/issue-radar/components/DashboardsSection", () => ({
  default: () => null,
}));
vi.mock("../apps/issue-radar/components/FiltersSection", () => ({
  default: () => null,
}));
vi.mock("../apps/issue-radar/components/PrFiltersSection", () => ({
  default: () => null,
}));
vi.mock("../apps/issue-radar/components/SettingsSection", () => ({
  default: () => null,
}));

const LeftRail = (await import("../apps/issue-radar/components/LeftRail"))
  .default;
...
describe("LeftRail", () => {
  ...
  it("applies the width it is given, defaulting to the former fixed width", () => {
    ...
    expect(
      (second.container.querySelector("aside") as HTMLElement).style.width,
    ).toBe("340px");
  });

  it("does not show the pull-request section for Jira", () => {
    ctx.value = {
      ...ctx.value,
      active: {
        owner: "PROJ",
...
describe("LeftRail — crews section", () => {
  ...
  it("carries no count badge — nothing on this surface queues for a human", () => {
    ...
    expect(
      container.querySelector('[data-testid="crews-needs-you"]'),
    ).toBeNull();
  ...
  it("carries the roster filters, with the server tally on each", async () => {
    ...
    render(<LeftRail />);
    expect(
      screen.getByTestId("crew-filter-all").getAttribute("aria-pressed"),
    ).toBe("true");
    expect(screen.getByTestId("crew-filter-working").textContent).toContain(
      "3",
    );
  ...
  it("cycles the roster sort from the rail", async () => {
    ...
    render(<LeftRail />);
    expect(
      screen.getByTestId("crew-sort-status").getAttribute("aria-pressed"),
    ).toBe("true");
...
describe("LeftRail — collapsed strip", () => {
  ...
  const entry = (push: boolean) => ({
    ...ACTIVE,
    permissions: { push, triage: false },
  });
  ...
  it("shows the full owner/repo turned on its side and drops the accordion", () => {
    ...
    expect(container.querySelector("aside > div")!.className).toContain(
      "rounded-xl",
    );
  });
  ...
  it("keeps the app mark but not its name — no room at 48px", () => {
    ...
    expect(
      container.querySelector('[title="Issue Radar v0.1.0"]'),
    ).toBeTruthy();
  });
  ...
  it("carries the read-only flag through the collapse, trailing the repo name", () => {
    ...
    expect(
      name.compareDocumentPosition(tag) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
