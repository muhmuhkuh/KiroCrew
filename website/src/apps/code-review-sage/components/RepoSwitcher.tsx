...
export default function RepoSwitcher() {
  ...
  const recentRows = useMemo(() => {
    ...
    return recent.filter((r) =>
      pinnedRepos.some((p) => repoSlug(p) === repoSlug(r)),
    );
  ...
  const rest = useMemo(() => {
    ...
    return filtered.filter(
      (r) => !recentRows.some((x) => repoSlug(x) === repoSlug(r)),
    );
  }, [filtered, recentRows]);
  ...
  const label = activeRepo
    ? repoSlug(activeRepo)
    : i18nT("apps.codeReviewSage.components.repoSwitcher.pick_a_repo");
        ...
        <DropdownMenuTrigger asChild>
          <button
            aria-label={
              activeRepo
                ? i18nT(
                    "apps.codeReviewSage.components.repoSwitcher.repository_named",
                    { name: label },
                  )
                : i18nT(
                    "apps.codeReviewSage.components.repoSwitcher.pick_a_repository",
                  )
            }
          ...
          >
            {activeRepo ? (
              activeRepo.provider === "gitlab" ? (
                <GitlabLogo size={18} className="flex-shrink-0" />
              ) : (
        ...
        >
          <DropdownMenuLabel className="text-[12px] uppercase tracking-[.04em]">
            {recentRows.length >= 2
              ...
              : i18nT(
                  "apps.codeReviewSage.components.repoSwitcher.repositories",
                )}
          </DropdownMenuLabel>
          ...
          {pinnedRepos.length > 6 && (
            <div className="px-2 pb-1.5">
              <input
                ...
                placeholder={i18nT(
                  "apps.codeReviewSage.components.repoSwitcher.filter_repos",
                )}
                aria-label={i18nT(
                  "apps.codeReviewSage.components.repoSwitcher.filter_repositories",
                )}
          ...
          {pinnedLoading && (
            <div className="px-3 py-2 text-[12.5px] text-muted">
              {i18nT("apps.codeReviewSage.components.repoSwitcher.loading")}
            </div>
          )}
          {!pinnedLoading && pinnedRepos.length === 0 && (
            <div className="px-3 py-2 text-[12.5px] text-muted leading-[1.5]">
              {i18nT(
                "apps.codeReviewSage.components.repoSwitcher.no_repos_yet_add_one_to_pick_from_the_repos_you",
              )}
            </div>
          )}
          {!pinnedLoading &&
            ...
            filtered.length === 0 && (
              <div className="px-3 py-2 text-[12.5px] text-muted">
                {i18nT(
                  "apps.codeReviewSage.components.repoSwitcher.no_repo_matches_that",
                )}
              </div>
            )}
            ...
            <>
              {recentRows.map((r) => {
                const isActive =
                  ...
                  (activeRepo.provider ?? "github") ===
                    (r.provider ?? "github") &&
                  (activeRepo.host ?? "github.com") ===
                    (r.host ?? "github.com");
                return (
                  <DropdownMenuItem
                    key={`recent-${repoSlug(r)}`}
                    onSelect={() => pick(r)}
                  >
                    <Clock
                      size={12}
          ...
          {rest.map((r) => {
            const isActive =
              ...
              activeRepo.owner === r.owner &&
              activeRepo.repo === r.repo &&
              (activeRepo.provider ?? "github") === (r.provider ?? "github") &&
              (activeRepo.host ?? "github.com") === (r.host ?? "github.com");
            return (
              <DropdownMenuItem key={repoSlug(r)} onSelect={() => pick(r)}>
                {r.provider === "gitlab" ? (
                  <GitlabLogo size={13} className="flex-shrink-0" />
                ) : (
                  <GithubLogo size={13} className="flex-shrink-0" />
                )}
                <span className="flex-1 min-w-0 truncate" title={repoSlug(r)}>
                  {r.owner ? `${r.owner}/` : ""}
                  <span className="font-medium">{r.repo}</span>
                </span>
                {isActive && (
          ...
          <DropdownMenuItem onSelect={() => openAddRepos()}>
            ...
            <span className="flex-1">
              {i18nT("apps.codeReviewSage.components.repoSwitcher.add_a_repo")}
            </span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
