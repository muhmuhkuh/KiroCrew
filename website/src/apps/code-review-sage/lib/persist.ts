...
import type { ActiveRepo } from "../context";
...
export function loadRecentRepos(): ActiveRepo[] {
  try {
    const raw = localStorage.getItem(RECENT_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    if (!parsed || parsed.v !== SCHEMA || !Array.isArray(parsed.repos))
      return [];
    return parsed.repos
      .filter(
        (r: unknown): r is ActiveRepo =>
          !!r &&
          typeof (r as { owner?: unknown }).owner === "string" &&
...
export function rememberRecentRepo(repo: ActiveRepo): ActiveRepo[] {
  const next = [
    { ...repo },
    ...loadRecentRepos().filter(
      (r) =>
        !(
          r.owner === repo.owner &&
          r.repo === repo.repo &&
          (r.provider ?? "github") === (repo.provider ?? "github") &&
          (r.host ?? "github.com") === (repo.host ?? "github.com")
        ),
    ),
  ].slice(0, MAX_RECENT);
  try {
    localStorage.setItem(
      RECENT_KEY,
      JSON.stringify({ v: SCHEMA, repos: next }),
    );
  } catch {
    /* best-effort */
  }
