...
export default function SetupPanel({
  ...
  const clone = useMutation({
    mutationFn: (u: string) =>
      postJson<CloneResult>("/setup-clone", { url: u }),
    onSuccess: (r) => {
      if (r.ok) {
    ...
    <Card>
      ...
      <div className="flex flex-col gap-2">
        <label className="text-[13px] text-muted">
          {i18nT("autoImprovement.repoLabel")}
        </label>
        <div className="flex items-center gap-2">
          <Input
            ...
            placeholder={i18nT(
              "apps.autoImprovement.setupPanel.https_github_com_owner_repo",
            )}
            className="flex-1"
            aria-label={i18nT("autoImprovement.repoLabel")}
            {...ime.bindEnter({
              onEnter: () => {
                if (url.trim()) clone.mutate(url.trim());
              },
            })}
          />
          <Btn
        ...
        {configured ? (
          <p className="text-[12px] text-muted">
            {i18nT("autoImprovement.currentRepo")}{" "}
        ...
        <div className="mt-3 flex flex-col gap-2">
          ...
          <label className="mt-1 flex items-start gap-2">
            <input
              ...
              onChange={(e) =>
                saveConfig.mutate({ directCommit: e.target.checked })
              }
              className="mt-0.5"
      ...
      {configured ? (
        <div className="mt-4 flex items-center gap-3 border-t border-border pt-3">
          {running ? (
            ...
            >
              <Square className="lucide-inline" size={14} />{" "}
              {i18nT("autoImprovement.stopBtn")}
            </Btn>
          ) : (
            <Btn
              primary
            ...
            >
              <Play className="lucide-inline" size={14} />{" "}
              {i18nT("autoImprovement.runBtn")}
            </Btn>
          )}
