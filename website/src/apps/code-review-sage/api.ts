...
export const sageApi = {
  ...
  chatState: (runId: string, changeId: string): Promise<ChatState> =>
    getJSON(
      `/chat?run_id=${encodeURIComponent(runId)}` +
  ...
  reviewLinks: (
    links: string,
  ): Promise<{ run_id: string; changes: string[] }> =>
  ...
  unpinRepo: (
    ...
    provider?: string,
    host?: string,
  ): Promise<{ repos: PinnedRepo[] }> =>
    sendJSON("/repos", "DELETE", {
      owner,
      repo,
  ...
  ): Promise<{ ok: boolean; settings: Settings }> =>
