...
const FiltersSection = (
  await import("../apps/issue-radar/components/FiltersSection")
).default;
const LabelPalette = (
  await import("../apps/issue-radar/components/LabelPalette")
).default;
...
beforeEach(() => {
  vi.clearAllMocks();
  ctx.value = {
    active: { owner: "acme", repo: "widget", provider: "github" },
    sortKey: "number",
    sortDir: "asc",
    cycleSort,
    stateFilter: "open",
...
describe("FiltersSection", () => {
  ...
  it("offers priority sorting for Jira only", async () => {
    ctx.value = {
      ...ctx.value,
      active: { owner: "LC", repo: "", provider: "jira" },
    };
    render(<FiltersSection />);
    await userEvent.click(screen.getByText("Priority"));
    expect(cycleSort).toHaveBeenCalledWith("priority");
  });

  it("multiselects Jira statuses", async () => {
    ...
    expect(toggleStatus).toHaveBeenCalledWith("To Do");
  });
...
describe("LabelPalette", () => {
  function renderPalette(
    over: Partial<React.ComponentProps<typeof LabelPalette>> = {},
  ) {
    return render(
      <LabelPalette
        labels={LABELS}
  ...
  it("keeps non-matching rows mounted but hidden, so the palette morphs", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Search labels" }),
    );
  ...
  it("clears the query when the search box is closed, so collapsed means unfiltered", async () => {
    ...
    await userEvent.click(
      screen.getAllByRole("button", { name: "Close label search" })[0],
    );
  ...
  it("says so when a query matches nothing", async () => {
    ...
    expect(screen.getByText(/nomatchxyz/)).toBeTruthy();
  });
  ...
  it("reports loading, error and empty states instead of an empty block", () => {
    ...
    const failed = renderPalette({
      labels: [],
      error: new Error("zzq-labels-broke"),
    });
    expect(screen.getByText("zzq-labels-broke")).toBeTruthy();
    failed.unmount();

    renderPalette({ labels: [] });
    expect(screen.getByText("No labels on this repo.")).toBeTruthy();
