...
import {
  useOptimisticConfigPaths,
  setConfigPathValue,
} from "./useOptimisticConfigPaths";
...
import {
  ...
  setBusySendDefault,
  type BusySendMode,
} from "../../components/BusySendButton";
...
function restoreLabels(): string[] {
  return [
    ...
    "12h",
    "24h",
    i18nT("pages.settings.chatPanel.no_limit"),
  ];
}
...
const VERBOSITY_OPTIONS: VerbosityLevel[] = [
  "default",
  "concise",
  "ultra",
  "answer_only",
...
export function ChatPanel() {
  ...
  const tipsOpts = overlay.mutationOpts<boolean>({
    ...
    mutationFn: (enable: boolean) =>
      api.tipsFeedback("", enable ? "optin" : "optout"),
    path: () => "tipsStatus.opted_out",
    displayValue: (enable) => !enable,
    applyToCache: (cached, enable) => ({
      ...(cached as { enabled_config: boolean; opted_out: boolean }),
      opted_out: !enable,
    }),
    onFailure: () =>
      setPathSaveError(
        "tipsStatus.opted_out",
        i18nT("pages.settings.chatPanel.failed_to_save_tips_preference"),
      ),
  ...
  const shownOptedOut = overlay.shown(
    "tipsStatus.opted_out",
    tipsQ.data?.opted_out,
  );
  ...
  const dashMut = useMutation(
    overlay.mutationOpts<Partial<DashboardConfig>>({
      queryKey: ["dashboardConfig"],
      mutationFn: (patch: Partial<DashboardConfig>) =>
        api.updateDashboardConfig(patch),
      path: () => "dashboardConfig",
      displayValue: (patch) => ({ ...dashCfg, ...patch }),
      applyToCache: (cached, patch) => ({
      ...
      onFailure: () =>
        setPathSaveError(
          "dashboardConfig",
          i18nT("pages.settings.chatPanel.failed_to_save_dashboard_config"),
        ),
      onSupersede: clearOwnPathError,
    }),
  );
  ...
  const optimisticConfigOpts = (
    ...
    overlay.mutationOpts<string>({
      ...
      path: () => path,
      displayValue: (v) => v,
      applyToCache: (cached, v) =>
        setConfigPathValue(cached as KirocrewConfigShape, path, v),
  ...
  const profileMut = useMutation({
    ...
    onError: () =>
      setSaveError(i18nT("pages.settings.chatPanel.failed_to_save_profile")),
  });
  ...
  const summaryMut = useMutation({
    ...
    onError: () =>
      setSaveError(
        i18nT("pages.settings.chatPanel.failed_to_save_session_summaries"),
      ),
  });
  ...
  const [busyDefault, setBusyDefaultState] = useState<BusySendMode>(() =>
    readBusySendDefault(),
  );
  const setBusyDefault = (m: BusySendMode) => {
    setBusySendDefault(m);
    setBusyDefaultState(m);
  };
  ...
  useEffect(() => {
    if (mcQ.data && !budgetInitRef.current) {
      ...
      setLocalBudget(
        String(mcQ.data.agent?.soft_stop_budget_secs ?? SOFT_STOP_DEFAULT),
      );
    }
  }, [mcQ.data]);
  ...
  const budgetMut = useMutation({
    mutationFn: (n: number) =>
      api.patchConfig("agent.soft_stop_budget_secs", n),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["kirocrewConfig"] }),
    onError: () => {
      setSaveError(
        i18nT("pages.settings.chatPanel.failed_to_save_soft_stop_budget"),
      );
      ...
      setLocalBudget(
        String(mcCfg?.agent?.soft_stop_budget_secs ?? SOFT_STOP_DEFAULT),
      );
    },
  });
  ...
  const shownFallbackModel = overlay.shown(
  ...
  const fallbackMut = useMutation(
    optimisticConfigOpts("agent.fallback_model", (err: unknown) => {
      ...
      return (
        i18nT("pages.settings.chatPanel.failed_to_save_fallback_model") + reason
      );
    }),
  );
  const fallbackModelOptions = (shown: string, server: string): string[] => {
    const opts = [
      "",
      "auto",
      ...availableModels.map((m) => m.name).filter((m) => m !== "auto"),
    ];
  ...
  useEffect(() => {
    if (mcQ.data && !keepCharsInitRef.current) {
      ...
      setLocalKeepChars(
        String(
          mcQ.data.agent?.completion_keep_chars ??
            COMPLETION_KEEP_CHARS_DEFAULT,
        ),
      );
    }
  }, [mcQ.data]);
  ...
  const keepCharsMut = useMutation({
    mutationFn: (n: number) =>
      api.patchConfig("agent.completion_keep_chars", n),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["kirocrewConfig"] }),
    onError: () => {
      setSaveError(
        i18nT(
          "pages.settings.chatPanel.failed_to_save_completion_keep_characters",
        ),
      );
      setLocalKeepChars(
        String(
          mcCfg?.agent?.completion_keep_chars ?? COMPLETION_KEEP_CHARS_DEFAULT,
        ),
      );
    },
  });
  ...
  const keepModeMut = useMutation({
    mutationFn: (v: CompletionKeepMode) =>
      api.patchConfig("agent.completion_keep", v),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["kirocrewConfig"] }),
    onError: () =>
      setSaveError(
        i18nT("pages.settings.chatPanel.failed_to_save_completion_keep_mode"),
      ),
  });
  ...
  const shownBackgroundModel = overlay.shown(
    "agent.role_models.background",
    backgroundModel,
  );
  const shownSubagentModel = overlay.shown(
  ...
  const roleModelLabels = (opts: string[]): string[] =>
    opts.map((m) =>
      m === "auto" ? i18nT("pages.settings.chatPanel.role_model_auto") : m,
    );
  ...
  const backgroundModelOpts = roleModelOptions(
  ...
  const backgroundModelMut = useMutation(
    optimisticConfigOpts("agent.role_models.background", () =>
      i18nT("pages.settings.chatPanel.failed_to_save_role_model"),
    ),
  );
  const subagentModelMut = useMutation(
    optimisticConfigOpts("agent.role_models.subagent", () =>
      i18nT("pages.settings.chatPanel.failed_to_save_role_model"),
    ),
  );
  ...
  const shownBackgroundEffort = overlay.shown(
    "agent.role_efforts.background",
    backgroundEffort,
  );
  const shownSubagentEffort = overlay.shown(
    "agent.role_efforts.subagent",
    subagentEffort,
  );
  const bgEffortSupported = modelSupportsEffort(
    shownBackgroundModel !== "auto" ? shownBackgroundModel : shownDefaultModel,
  );
  const subEffortSupported = modelSupportsEffort(
    shownSubagentModel !== "auto" ? shownSubagentModel : shownDefaultModel,
  );
  const effortLabels = EFFORT_LEVELS.map((l) =>
    l === "" ? i18nT("pages.settings.chatPanel.model_default") : effortLabel(l),
  );
  const backgroundEffortMut = useMutation(
    optimisticConfigOpts("agent.role_efforts.background", () =>
      i18nT("pages.settings.chatPanel.failed_to_save_role_effort"),
    ),
  );
  const subagentEffortMut = useMutation(
    optimisticConfigOpts("agent.role_efforts.subagent", () =>
      i18nT("pages.settings.chatPanel.failed_to_save_role_effort"),
    ),
  );
  ...
  const setChat = useCallback(
    <K extends keyof ChatConfig>(k: K, v: ChatConfig[K]) => {
      setChatCfg((prev) => {
        const next = { ...prev, [k]: v };
        saveChatConfig(next);
        return next;
    ...
    <>
      ...
      {dashQ.isError && (
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <ErrorNotice
            ...
            message={i18nT(
              "pages.settings.chatPanel.failed_to_load_dashboard_config",
            )}
          />
          <Btn onClick={() => dashQ.refetch()}>
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.model")}>
        ...
        <SettingsCard>
          <div className="text-[13px] font-semibold text-text-strong">
            {i18nT("pages.settings.chatPanel.role_chat")}
          </div>
          <SettingsSelect
            label={i18nT("pages.settings.chatPanel.default_model")}
            description={i18nT(
              "pages.settings.chatPanel.which_model_new_sessions_start_with_pick_a_model",
            )}
            hint={i18nT(
              "pages.settings.chatPanel.default_defers_to_your_agent_config_and_then_to",
            )}
            value={shownDefaultModel}
            options={modelOptions}
            optionLabels={modelOptions.map((m) =>
              m === "auto" ? i18nT("pages.settings.chatPanel.default_auto") : m,
            )}
          ...
          <SettingsSelect
            ...
            description={i18nT(
              "pages.settings.chatPanel.how_long_models_think_before_answering_by_defaul",
            )}
            hint={
              effortSupported
                ? i18nT(
                    "pages.settings.chatPanel.model_default_applies_no_override_the_model_pick",
                  )
        ...
        <SettingsCard index={1}>
          <div className="text-[13px] font-semibold text-text-strong">
            {i18nT("pages.settings.chatPanel.role_background")}
          </div>
          <div className="text-[12px] text-muted -mt-0.5">
            {i18nT(
              "pages.settings.chatPanel.model_for_background_lite_heartbeat_work",
            )}
          </div>
          <SettingsSelect
        ...
        <SettingsCard index={2}>
          <div className="text-[13px] font-semibold text-text-strong">
            {i18nT("pages.settings.chatPanel.role_subagents")}
          </div>
          <div className="text-[12px] text-muted -mt-0.5">
            {i18nT("pages.settings.chatPanel.model_for_spawned_sub_agents")}
          </div>
          <SettingsSelect
        ...
        <SettingsCard>
          <div className="text-[13px] font-semibold text-text-strong">
            {i18nT("pages.settings.chatPanel.throttle_fallback")}
          </div>
          <div className="text-[12px] text-muted -mt-0.5">
            {i18nT(
              "pages.settings.chatPanel.model_tried_when_your_current_model_stays_rate_li",
            )}
          </div>
          <SettingsSelect
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.about_you")}>
        <SettingsCard index={3}>
          <SettingsSelect
            ...
            description={i18nT(
              "pages.settings.chatPanel.kiro_matches_vocabulary_and_examples_to_your_pro",
            )}
            value={userRole}
            options={ROLE_OPTIONS}
            optionLabels={roleLabels()}
            onChange={(v) =>
              profileMut.mutate({ path: "dashboard.user_role", value: v })
            }
          />
          {userRole === "other" && (
            <SettingsInput
              ...
              description={i18nT(
                "pages.settings.chatPanel.kiro_quotes_this_back_to_itself_when_calibrating",
              )}
              placeholder={i18nT(
                "pages.settings.chatPanel.e_g_solutions_architect_sre_founder",
              )}
          ...
          <SettingsSelect
            ...
            description={i18nT(
              "pages.settings.chatPanel.sets_how_deep_explanations_go_plain_language_vs",
            )}
            value={userTechLevel}
            ...
            onChange={(v) =>
              profileMut.mutate({
                path: "dashboard.user_technical_level",
                value: v,
              })
            }
          />
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.power")}>
        <SettingsCard index={4}>
          <SettingsToggle
            label={i18nT(
              "pages.settings.chatPanel.prevent_sleep_while_running",
            )}
            description={i18nT(
              "pages.settings.chatPanel.keep_your_computer_awake_while_a_task_is_running",
            )}
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.composer")}>
        <SettingsCard index={5}>
          <SettingsSelect
            ...
            description={
              chatCfg.sendOnEnter === "enter"
                ? i18nT("pages.settings.chatPanel.shift_enter_for_newline")
                : chatCfg.sendOnEnter === "ctrl-enter"
                  ? i18nT("pages.settings.chatPanel.enter_for_newline")
                  : i18nT("pages.settings.chatPanel.mod_enter_for_newline", {
                      mod: isMac ? "⌘" : "Ctrl",
                    })
            }
            ...
            optionLabels={[
              i18nT("pages.settings.chatPanel.enter_sends"),
              i18nT("pages.settings.chatPanel.mod_enter_sends", {
                mod: isMac ? "⌘" : "Ctrl",
              }),
              i18nT("pages.settings.chatPanel.enter_sends_mod_enter_newline", {
                mod: isMac ? "⌘" : "Ctrl",
              }),
            ]}
          ...
          <SettingsButtonGroup
            label={i18nT(
              "pages.settings.chatPanel.what_enter_does_while_the_agent_is_working",
            )}
            description={
              chatCfg.sendOnEnter === "enter"
                ? i18nT("pages.settings.chatPanel.busy_alt_action_desc", {
                    chord: platformShortcut("Cmd+Enter"),
                  })
                : i18nT(
                    "pages.settings.chatPanel.busy_alt_action_desc_no_chord",
                  )
            }
            value={busyDefault}
            options={[
          ...
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.quick_send")}
            description={i18nT(
              "pages.settings.chatPanel.click_a_suggested_reply_to_send_it_instantly",
              { mod: isMac ? "⇧" : "Shift" },
            )}
            checked={dashCfg.quick_send}
            onChange={(v) => setDash({ quick_send: v })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.merge_queued_messages")}
            description={i18nT(
              "pages.settings.chatPanel.combine_follow_up_messages_into_a_single_labeled",
            )}
            checked={dashCfg.merge_queued_messages}
            onChange={(v) => setDash({ merge_queued_messages: v })}
            disabled={dashDisabled}
          />
          <SettingsButtonGroup
            label={i18nT("pages.settings.chatPanel.follow_up_bar_layout")}
            description={i18nT(
              "pages.settings.chatPanel.multiline_wraps_suggestions_onto_multiple_rows_s",
            )}
            value={chatCfg.followUpLayout}
            options={[
              {
                value: "multiline",
                label: i18nT("pages.settings.chatPanel.multiline"),
              },
              {
                value: "scroll",
                label: i18nT("pages.settings.chatPanel.single_line"),
              },
            ]}
            onChange={(v) =>
              setChat("followUpLayout", v as ChatConfig["followUpLayout"])
            }
          />
          <SettingsInput
            label={i18nT("pages.settings.chatPanel.soft_stop_budget_seconds")}
            aria-label={i18nT(
              "pages.settings.chatPanel.soft_stop_budget_seconds",
            )}
            hint={i18nT(
              "pages.settings.chatPanel.how_long_to_wait_for_the_agent_to_honor_a_stop_p",
            )}
            type="number"
            value={localBudget}
            min={SOFT_STOP_MIN}
            ...
            onBlur={() => {
              ...
              if (isNaN(n) || n < SOFT_STOP_MIN || n > SOFT_STOP_MAX) {
                setLocalBudget(
                  String(
                    mcCfg?.agent?.soft_stop_budget_secs ?? SOFT_STOP_DEFAULT,
                  ),
                );
                return;
              }
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.messages")}>
        <SettingsCard index={6}>
          <SettingsButtonGroup
            ...
            description={i18nT(
              "pages.settings.chatPanel.immediate_mode_shows_raw_chunks_as_they_arrive_s",
            )}
            value={chatCfg.streamMode}
            options={[
              {
                value: "immediate",
                label: i18nT("pages.settings.chatPanel.immediate"),
              },
              {
                value: "smooth",
                label: i18nT("pages.settings.chatPanel.smooth"),
              },
            ]}
            onChange={(v) =>
              setChat("streamMode", v as ChatConfig["streamMode"])
            }
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.show_timestamps")}
            description={i18nT(
              "pages.settings.chatPanel.display_time_on_each_message",
            )}
            checked={chatCfg.showTimestamps}
            onChange={(v) => setChat("showTimestamps", v)}
          />
          <SettingsButtonGroup
            label={i18nT("pages.settings.chatPanel.content_width")}
            description={i18nT(
              "pages.settings.chatPanel.compact_is_the_original_view_comfortable_and_ful",
            )}
            value={chatCfg.contentWidth}
            options={[
              {
                value: "compact",
                label: i18nT("pages.settings.chatPanel.compact"),
              },
              {
                value: "comfortable",
                label: i18nT("pages.settings.chatPanel.comfortable"),
              },
              { value: "full", label: i18nT("pages.settings.chatPanel.full") },
            ]}
            onChange={(v) => setChat("contentWidth", v as ContentWidth)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.show_thinking_inline")}
            description={i18nT(
              "pages.settings.chatPanel.show_intermediate_reasoning_text_between_tool_ca",
            )}
            checked={!chatCfg.collapseAllSteps}
            onChange={(v) => setChat("collapseAllSteps", !v)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.pin_last_prompt")}
            description={i18nT("pages.settings.chatPanel.pin_last_prompt_desc")}
            checked={chatCfg.pinLastPrompt}
            onChange={(v) => setChat("pinLastPrompt", v)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.simplified_tool_call_names")}
            description={i18nT(
              "pages.settings.chatPanel.when_enabled_inline_tool_pills_show_simplified_t",
            )}
            checked={chatCfg.simplifiedToolNames}
            onChange={(v) => setChat("simplifiedToolNames", v)}
          />
          <SettingsSelect
            label={i18nT("pages.settings.chatPanel.file_change_chips")}
            description={i18nT(
              "pages.settings.chatPanel.how_file_diff_chips_appear_below_assistant_messa",
            )}
            value={chatCfg.fileChipStyle}
            options={["expanded", "minimal"]}
            optionLabels={[
              i18nT("pages.settings.chatPanel.expanded_icon_name_stats"),
              i18nT(
                "pages.settings.chatPanel.minimal_stats_only_name_on_hover",
              ),
            ]}
            onChange={(v) =>
              setChat("fileChipStyle", v as ChatConfig["fileChipStyle"])
            }
          />
          ...
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.link_previews")}
            description={i18nT(
              "pages.settings.chatPanel.show_a_favicon_and_page_title_instead_of_the_raw",
            )}
            checked={dashCfg.link_previews}
            onChange={(v) => setDash({ link_previews: v })}
            disabled={dashDisabled}
          />
          <SettingsSelect
            label={i18nT("pages.settings.chatPanel.widget_density")}
            description={i18nT(
              "pages.settings.chatPanel.how_aggressively_the_agent_uses_inline_widgets_f",
            )}
            value={dashCfg.widget_density ?? "more"}
            options={["more", "less"]}
            optionLabels={[
              i18nT("pages.settings.chatPanel.more_encourage_widgets"),
              i18nT("pages.settings.chatPanel.less_only_when_needed"),
            ]}
            onChange={(v) => setDash({ widget_density: v as "more" | "less" })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.mcp_apps_in_side_panel")}
            description={i18nT(
              "pages.settings.chatPanel.render_interactive_mcp_apps_in_the_right_side_pa",
            )}
            checked={dashCfg.mcp_app_panel}
            onChange={(v) => setDash({ mcp_app_panel: v })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.auto_open_git_panel")}
            description={i18nT(
              "pages.settings.chatPanel.expand_the_side_panel_to_the_git_tab_each_time_yo",
            )}
            checked={dashCfg.auto_open_git_panel}
            onChange={(v) => setDash({ auto_open_git_panel: v })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.session_card_source_links")}
            description={i18nT(
              "pages.settings.chatPanel.session_card_source_links_desc",
            )}
            checked={dashCfg.session_card_source_links}
            onChange={(v) => setDash({ session_card_source_links: v })}
            disabled={dashDisabled}
          />
          <SettingsSelect
            label={i18nT("pages.settings.chatPanel.response_verbosity")}
            description={i18nT(
              "pages.settings.chatPanel.how_terse_the_agent_s_prose_is_ultra_concise_cap",
            )}
            value={asVerbosity(dashCfg.verbosity)}
            options={VERBOSITY_OPTIONS}
            optionLabels={[
              i18nT("pages.settings.chatPanel.default_normal_length"),
              i18nT("pages.settings.chatPanel.concise_trim_filler"),
              i18nT("pages.settings.chatPanel.ultra_concise_3_sentences"),
              i18nT("pages.settings.chatPanel.answer_only_details_on_request"),
            ]}
            onChange={(v) => setDash({ verbosity: v as VerbosityLevel })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.show_context_percentage")}
            description={i18nT(
              "pages.settings.chatPanel.display_usage_percentage_next_to_the_context_pro",
            )}
            checked={chatCfg.showContextPct}
            onChange={(v) => setChat("showContextPct", v)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.show_token_usage")}
            description={i18nT(
              "pages.settings.chatPanel.display_used_and_total_tokens_next_to_the_contex",
            )}
            checked={chatCfg.showContextTokens}
            onChange={(v) => setChat("showContextTokens", v)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.feature_tips")}
            description={
              tipsConfigOff
                ? i18nT(
                    "pages.settings.chatPanel.disabled_by_instance_config_tips_enabled_false",
                  )
                : i18nT(
                    "pages.settings.chatPanel.show_occasional_feature_discovery_tips_above_the",
                  )
            }
            checked={
              !!tipsQ.data && tipsQ.data.enabled_config && !shownOptedOut
            }
            onChange={(v) => tipsMut.mutate(v)}
            disabled={tipsConfigOff || tipsQ.isLoading || tipsQ.isError}
          />
          ...
          <ErrorNotice
            ...
            message={
              tipsQ.isError
                ? i18nT(
                    "pages.settings.chatPanel.failed_to_load_tips_preference",
                  )
                : null
            }
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.folder_suggestions")}
            description={i18nT(
              "pages.settings.chatPanel.offer_to_file_a_new_session_into_a_matching_fold",
            )}
            checked={dashCfg.folder_suggestions_enabled}
            onChange={(v) => setDash({ folder_suggestions_enabled: v })}
            disabled={dashDisabled}
          />
        </SettingsCard>
      </SettingsSection>
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.sessions")}>
        <SettingsCard index={7}>
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.split_view_session_grid")}
            description={i18nT(
              "pages.settings.chatPanel.opt_in_split_the_chat_into_resizable_session_pan",
              { mod: isMac ? "⌘" : "Ctrl" },
            )}
            checked={dashCfg.session_grid}
            onChange={(v) => setDash({ session_grid: v })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.history_expanded")}
            description={i18nT(
              "pages.settings.chatPanel.expand_history_sidebar_by_default",
            )}
            checked={chatCfg.historyExpanded}
            onChange={(v) => setChat("historyExpanded", v)}
          />
          <SettingsToggle
            label={i18nT(
              "pages.settings.chatPanel.confirm_before_closing_session",
            )}
            description={i18nT(
              "pages.settings.chatPanel.show_a_confirmation_dialog_when_closing_a_sessio",
            )}
            checked={chatCfg.confirmCloseSession}
            onChange={(v) => setChat("confirmCloseSession", v)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.default_to_autopilot_mode")}
            description={i18nT(
              "pages.settings.chatPanel.new_sessions_start_in_autopilot_mode_plan_approv",
            )}
            checked={chatCfg.defaultAutopilot}
            onChange={(v) => setChat("defaultAutopilot", v)}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.tail_only_fork")}
            description={i18nT(
              "pages.settings.chatPanel.fork_keeps_only_the_messages_after_the_chosen_po",
            )}
            checked={dashCfg.tail_fork_enabled}
            onChange={(v) => setDash({ tail_fork_enabled: v })}
            disabled={dashDisabled}
          />
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.restore_sessions")}
            description={i18nT(
              "pages.settings.chatPanel.re_open_recently_active_sessions_on_startup",
            )}
            checked={dashCfg.restore_sessions}
            onChange={(v) => setDash({ restore_sessions: v })}
            disabled={dashDisabled}
          />
          {dashCfg.restore_sessions && (
            <SettingsSelect
              label={i18nT("pages.settings.chatPanel.restore_window")}
              description={i18nT(
                "pages.settings.chatPanel.time_window_for_session_restoration",
              )}
              value={String(dashCfg.restore_window_minutes)}
              options={RESTORE_OPTIONS}
              optionLabels={restoreLabels()}
              onChange={(v) => setDash({ restore_window_minutes: Number(v) })}
              disabled={dashDisabled}
            />
          )}
          <SettingsToggle
            label={i18nT("pages.settings.chatPanel.session_summaries")}
            description={i18nT(
              "pages.settings.chatPanel.summarize_each_session_by_intent_in_the_right_pa",
            )}
            checked={summaryEnabled}
            onChange={(v) => summaryMut.mutate(v)}
            disabled={!mcQ.isSuccess || summaryMut.isPending}
          />
        </SettingsCard>
      </SettingsSection>
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.context")}>
        <SettingsCard index={8}>
          <SettingsSelect
            ...
            description={i18nT(
              "pages.settings.chatPanel.context_usage_at_which_auto_compaction_triggers",
            )}
            ...
            onChange={(v) =>
              api
                .patchConfig("session.autocompact_pct", Number(v))
                .then(() =>
                ...
                .catch(() =>
                  setSaveError(
                    i18nT(
                      "pages.settings.chatPanel.failed_to_save_auto_compact_threshold",
                    ),
                  ),
                )
            }
      ...
      <SettingsSection title={i18nT("pages.settings.chatPanel.subagents")}>
        <SettingsCard index={10}>
          <SettingsSelect
            label={i18nT(
              "pages.settings.chatPanel.completion_event_truncation",
            )}
            description={i18nT(
              "pages.settings.chatPanel.which_part_of_a_subagent_s_stream_to_keep_when_i",
            )}
          ...
          <SettingsInput
            label={i18nT(
              "pages.settings.chatPanel.completion_event_characters",
            )}
            aria-label={i18nT(
              "pages.settings.chatPanel.completion_event_characters_2",
            )}
            hint={i18nT(
              "pages.settings.chatPanel.maximum_characters_retained_in_the_completion_ev",
              { n: COMPLETION_KEEP_CHARS_DEFAULT },
            )}
            type="number"
            ...
            onBlur={() => {
              ...
              if (
                ...
                setLocalKeepChars(
                  String(
                    mcCfg?.agent?.completion_keep_chars ??
                      COMPLETION_KEEP_CHARS_DEFAULT,
                  ),
                );
                return;
