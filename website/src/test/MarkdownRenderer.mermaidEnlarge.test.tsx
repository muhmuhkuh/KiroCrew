...
async function enlarge(trigger: HTMLElement) {
  await userEvent.click(trigger)
  await userEvent.click(await screen.findByRole('menuitem', { name: /enlarge diagram/i }))
...
describe('MermaidBlock click-to-enlarge', () => {
  ...
  it('closes the overlay when a re-render of the diagram fails', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('mermaid-more-actions')).toBeTruthy()
    )
    expect(screen.queryByRole('dialog')).toBeNull()
  })
