...
import {
  ...
  runCoversChange,
  sageSourceLink,
} from "../apps/code-review-sage/lib/format";
...
describe("matching a PR to its run", () => {
  ...
  it("matches the same PR across trailing-slash and case differences", () => {
    ...
    expect(
      runCoversChange(run, "https://github.com/acme/widgets/pull/7/"),
    ).toBe(true);
    expect(runCoversChange(run, "https://GitHub.com/acme/widgets/pull/7")).toBe(
      true,
    );
    expect(
      runCoversChange(run, "  https://github.com/acme/widgets/pull/7  "),
  ...
  it("does not match a different PR number in the same repo", () => {
    const run = { changes: ["https://github.com/acme/widgets/pull/7"] };
    expect(
      runCoversChange(run, "https://github.com/acme/widgets/pull/70"),
    ).toBe(false);
    expect(runCoversChange(run, "https://github.com/acme/widgets/pull/8")).toBe(
      false,
    );
  });
  ...
  it("changeKey normalizes only case and trailing slashes", () => {
    expect(changeKey("https://github.com/o/r/pull/1/")).toBe(
    ...
    expect(changeKey("https://github.com/acme/service-api/pull/5")).not.toBe(
      changeKey("https://github.com/acme/service_api/pull/5"),
    );
  });

  it("builds a shared source link for self-hosted GitLab MRs", () => {
    const url =
      "https://gitlab.bildungsinnovator.com/bildungsinnovator/lxt/vox/-/merge_requests/1141";
    expect(sageSourceLink(url)).toEqual({
      url,
      provider: "gitlab",
      number: 1141,
      repo: "vox",
