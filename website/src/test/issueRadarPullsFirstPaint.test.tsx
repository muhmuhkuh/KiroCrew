...
function State() {
  ...
  return (
    <div>
      <div data-testid="s">
        {pullsLoading ? "loading" : "ready"}:{rows.length}:
        {pullsPartial ? "partial" : "full"}
      </div>
      <div data-testid="view">{mainView}</div>
      <button data-testid="to-pulls" onClick={() => openPulls()}>
        pulls
      </button>
      <button
        data-testid="to-closed"
        onClick={() => setPrStateFilter("closed")}
      >
        closed
      </button>
    </div>
...
function renderProvider(
  seed?: unknown,
  repo: typeof REPO | typeof JIRA_REPO = REPO,
) {
  ...
  if (seed !== undefined) {
    client.setQueryData(
      ["issue-radar", "pulls", "github:github.com:kirodotdev/Kiro", "open"],
      seed,
    );
  }
  render(
...
describe("progressive PR first paint", () => {
  it("paints the first page while the full fetch is in flight, then swaps to the full set", async () => {
    ...
    await act(async () => {
      screen.getByTestId("to-pulls").click();
    });

    // First page painted: one row, marked partial, skeleton gone.
    await waitFor(() =>
      expect(screen.getByTestId("s").textContent).toBe("ready:1:partial"),
    );
    ...
    await waitFor(() =>
      expect(screen.getByTestId("s").textContent).toBe("ready:2:full"),
    );
  });
  ...
  it("does not leak the open first page into the Closed tab", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("s").textContent).toBe("loading:0:full"),
    );
  ...
  it("does not fire a first-page request on a warm open (full list already resident)", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("s").textContent).toBe("ready:1:full"),
    );
  ...
  it("does not request or open a pull-request surface for Jira", async () => {
    renderProvider(undefined, JIRA_REPO);
    await act(async () => {
      screen.getByTestId("to-pulls").click();
    });
