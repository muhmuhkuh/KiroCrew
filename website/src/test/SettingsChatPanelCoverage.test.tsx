...
vi.mock(
  ...
  async () => await import("./__mocks__/@radix-ui/react-select"),
);
...
beforeEach(() => {
  ...
  dashboardConfigMock.mockImplementation(
    () => Promise.resolve({ ...BASE_DASH }) as never,
  );
  updateDashboardConfigMock.mockImplementation(
  ...
  tipsFeedbackMock.mockImplementation(
...
describe("ChatPanel — load failures", () => {
  it("surfaces a dashboard-config load failure and refetches on Retry", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText("Failed to load dashboard config."),
  ...
  it("surfaces a config load failure and refetches on Retry", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText("Failed to load config."),
      ).not.toBeInTheDocument(),
    );
  });
...
describe("ChatPanel — save-error banner", () => {
  it("rolls the optimistic write back and explains the failure", async () => {
    ...
    fireEvent.click(sw);
    expect(
      await screen.findByText(/Failed to save dashboard config/),
    ).toBeInTheDocument();
  ...
  it("clears the banner when dismissed", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText(/Failed to save dashboard config/),
      ).not.toBeInTheDocument(),
    );
  });
});
...
describe("ChatPanel — Composer", () => {
  ...
  it("reverts the soft-stop budget to the server value when the write fails", async () => {
    ...
    fireEvent.blur(input);
    expect(
      await screen.findByText(/Failed to save soft-stop budget/),
    ).toBeInTheDocument();
    await waitFor(() => expect(input.value).toBe("10"));
  });
});
...
describe("ChatPanel — Messages", () => {
  ...
  it("rolls the Feature Tips preference back when the write fails", async () => {
    ...
    fireEvent.click(sw);
    expect(
      await screen.findByText(/Failed to save tips preference/),
    ).toBeInTheDocument();
    await waitFor(() => expect(sw).toHaveAttribute("aria-checked", "true"));
  });
});
...
describe("ChatPanel — Sessions", () => {
  ...
  it("hides the restore window until session restore is on", async () => {
    ...
    expect(
      screen.queryByRole("combobox", { name: "Restore Window" }),
    ).not.toBeInTheDocument();
  });
...
describe("ChatPanel — Context", () => {
  ...
  it("surfaces a failed auto-compact write", async () => {
    ...
    expect(
      await screen.findByText(/Failed to save auto-compact threshold/),
    ).toBeInTheDocument();
  });
});
...
describe("ChatPanel — Subagents", () => {
  ...
  it("surfaces a failed completion-keep-mode write", async () => {
    ...
    expect(
      await screen.findByText(/Failed to save completion-keep mode/),
    ).toBeInTheDocument();
  });
  ...
  it("reverts the completion-keep characters when the write fails", async () => {
    ...
    fireEvent.blur(input);
    expect(
      await screen.findByText(/Failed to save completion-keep characters/),
    ).toBeInTheDocument();
    await waitFor(() => expect(input.value).toBe("3000"));
  });
});
...
describe("ChatPanel — per-role models", () => {
  it.each([
    ...
    await waitFor(() =>
      expect(patchConfigMock).toHaveBeenCalledWith(path, "claude-opus-4.8"),
    );
  });
  ...
  it.each([
    ["Background Model"],
    ["Subagent Model"],
  ])("%s surfaces a failed write", async (label) => {
    rejectOnce(patchConfigMock);
    wrap();
    await waitFor(() => expect(modelsMock).toHaveBeenCalled());
    await openSelect(label);
    ...
    expect(
      await screen.findByText(/Failed to save role model/),
    ).toBeInTheDocument();
  });
  ...
  it('keeps the chat row on its own "Default (auto)" spelling', async () => {
    ...
    expect(opts.map((o) => o.textContent)).not.toContain(
      "Auto (provider picks)",
    );
  });
  ...
  it("keeps a pinned role model selectable when the backend stops listing it", async () => {
    ...
    seedMc({
      agent: { role_models: { background: "claude-opus-4.7-retired" } },
    });
    wrap();
    await waitFor(() => expect(modelsMock).toHaveBeenCalled());
    const opts = await openSelect("Background Model");
...
describe("ChatPanel — per-role reasoning effort", () => {
  it.each([
    ...
    await waitFor(() =>
      expect(patchConfigMock).toHaveBeenCalledWith(path, "high"),
    );
  });
  ...
  it.each([
    ...
    expect(
      await screen.findByText(/Failed to save role effort/),
    ).toBeInTheDocument();
  });
  ...
  it.each([
    ["Background Effort"],
    ["Subagent Effort"],
  ])("%s is inert while the role inherits a non-reasoning chat default", async (label) => {
...
describe("ChatPanel — optimistic model selection (#6848)", () => {
  ...
  it("shows a picked reasoning effort immediately, before the PATCH settles", async () => {
    ...
    fireEvent.click(screen.getByRole("option", { name: "High" }));
    const trigger = screen.getByLabelText("Default Reasoning Effort");
  ...
  it("rolls the selector back to the server value when the PATCH fails", async () => {
    ...
    fireEvent.click(screen.getByRole("option", { name: "claude-haiku-4.5" }));
    expect(
      await screen.findByText(/Failed to save default model/),
    ).toBeInTheDocument();
  ...
  it("rolls a role model back when the PATCH fails", async () => {
    ...
    const trigger = screen.getByLabelText("Background Model");
