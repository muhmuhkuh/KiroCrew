...
- `"pi"` (`ACP_BACKEND_PI`): `pi-acp` launched through `pi_support.py`, which supplies the selected prompt, a per-session direct-tool `pi-mcp-adapter` configuration, and Kiro Crew's approval extension. Pi uses opaque session IDs, selects its native provider-qualified model IDs through `session/set_config_option`, and runs one `AcpClient` subprocess per session. Its adapter confirmation is correlated to the preceding direct tool call before the standard permission, hook, and approval path runs; unknown or reused correlations are refused. Pi does not receive the Kiro-only Tool Search overlay.
